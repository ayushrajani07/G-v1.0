...
Responsibilities:
  ...
  * Create default config if file missing (fallback behavior).
...
Public API:
  ...
  load_config(path: str, metrics=None) -> ConfigWrapper
...
def create_default_config() -> dict[str, Any]:
    ...
    Merged from legacy config_loader.py during Phase 2 consolidation.
    """
    return {
        "metrics": {
            "enabled": True,
        ...
        "indices": {
            ...
            "BANKNIFTY": {
                "enable": True,
                "expiries": ["this_month", "next_month"],
                "strikes_otm": 10,
                "strikes_itm": 10
            },
            "FINNIFTY": {
...
def load_and_validate_config(path: str | os.PathLike[str], *, metrics: Any = None) -> dict[str, Any]:
    ...
    Args:
        ...
        metrics: Optional metrics object for deprecated key tracking
    ...
    Environment Flags:
        G6_CONFIG_STRICT=1         - Escalate legacy usage to error
        G6_CONFIG_LEGACY_SOFT=1    - Allow legacy keys with warnings
        G6_CONFIG_EMIT_NORMALIZED=1 - Write normalized config to logs/
        G6_CONFIG_AUTO_CREATE=1    - Create default config if missing
    """
    # Ensure we see latest environment mutations (tests monkeypatch flags between calls)
    try:
        EnvConfig.clear_cache()
    except Exception:
        pass
    path_obj = Path(path)
    strict = EnvConfig.get_bool('G6_CONFIG_STRICT', False)
    soft_legacy = EnvConfig.get_bool('G6_CONFIG_LEGACY_SOFT', False)
    auto_create = EnvConfig.get_bool('G6_CONFIG_AUTO_CREATE', False)
    
    # Handle missing config file (merged from legacy config_loader.py)
    if not path_obj.exists():
        logger.warning("Configuration file not found: %s", path)
        
        if auto_create:
            # Create default config with auto-creation enabled
            ...
            try:
                ...
                # Return default without writing if directory creation fails
            ...
            # Without auto-create, raise error for missing file
            raise ConfigValidationError(f"Configuration file not found: {path} (set G6_CONFIG_AUTO_CREATE=1 to auto-create)")
    
    # Handle invalid JSON (merged from legacy config_loader.py)
    try:
        cfg = validate_config_file(path, strict=strict, soft_legacy=soft_legacy, metrics=metrics)
    except json.JSONDecodeError as e:
        logger.error("Invalid JSON in configuration file: %s", path)
        if auto_create:
            logger.warning("Returning default configuration due to JSON error")
    ...
    if EnvConfig.get_bool('G6_CONFIG_EMIT_NORMALIZED', False):
    ...
    if EnvConfig.get_bool('G6_CONFIG_VALIDATE_CAPABILITIES', False):
...
__all__ = [
    ...
    "ConfigError",
    "create_default_config",  # Exported for testing/utilities
]
