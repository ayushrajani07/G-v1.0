...
class Subscriber:
    ...
    def poll(self, batch_size: int = 100, timeout: float = 0.0):
        ...
        out: list[Event] = []
        while True:
            # Use condition variable to avoid busy-waiting when no new events
            with self._bus._cond:
                ...
                if remaining <= 0:
                    break
                # Wait for either a publish() notification or timeout
                self._bus._cond.wait(timeout=min(0.1, remaining))
        return out
...
class InMemoryBus:
    ...
    def publish(self, event_type: str, payload: dict, key: str | None = None, meta: dict | None = None) -> int:
        ...
        with self._lock:
            ...
            _emit_core_metrics()
            # Notify any waiting subscribers that new events are available
            try:
