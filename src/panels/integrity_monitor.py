...
def run_integrity_check_once(panels_dir: str | None = None) -> dict[str, int]:
    ...
    """
    panels_dir = panels_dir or EnvConfig.get_str('G6_PANELS_DIR', 'data/panels')
    ...
    strict = EnvConfig.get_bool('G6_PANELS_INTEGRITY_STRICT', False)
    if count:
...
def start_panels_integrity_monitor(panels_dir: str | None = None) -> None:
    ...
    if not EnvConfig.get_bool('G6_PANELS_INTEGRITY_MONITOR', False):
        return
    if _MONITOR_THREAD and _MONITOR_THREAD.is_alive():
        return
    panels_dir = panels_dir or EnvConfig.get_str('G6_PANELS_DIR', 'data/panels')
    interval = EnvConfig.get_float('G6_PANELS_INTEGRITY_INTERVAL', 300.0)
