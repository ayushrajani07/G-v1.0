...
def retry(
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            def _retry_sleep_and_log(attempt_idx: int, err: BaseException) -> None:
                nonlocal current_delay
                # Calculate delay with optional jitter
                sleep_time = current_delay * (0.5 + random.random()) if jitter else current_delay
                logger.warning(
            ...
            def _call_catching() -> tuple[bool, Any | None, BaseException | None]:
                try:
            ...
            for attempt in range(max_attempts):
                ok, result, err = _call_catching()
                if ok:
                    return result
                last_exception = err
                if attempt < max_attempts - 1 and err is not None:
                    _retry_sleep_and_log(attempt, err)
                elif err is not None:
                    logger.error(
                        "Failed after %s attempts for %s: %s",
                        max_attempts,
                        func.__name__,
                        str(err),
                    )
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            try:
                ...
                logger.warning("Fallback triggered for %s: %s", func.__name__, str(e))
