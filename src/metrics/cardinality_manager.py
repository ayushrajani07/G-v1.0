...
# Optional imports
try:
    from src.config.loader import get_loaded_config  # type: ignore
except ImportError:
    get_loaded_config = None  # type: ignore
    ...
    def should_emit(self, index: str, expiry: str, strike: int | float, opt_type: str,
        ...
        try:
            ...
            if callable(local_glc):
                ...
                if get_loaded_config:
                    cfg = get_loaded_config()
                else:
                    raise ImportError("get_loaded_config not available")
