...
# Optional imports
try:
    from src.tools.token_providers import get_provider  # type: ignore
except ImportError:
...
def _kite_validate_token(api_key: str, access_token: str) -> bool:
    ...
    if not api_key or not access_token:
        ...
        return False
    verbose = EnvConfig.get_bool("G6_KITE_AUTH_VERBOSE", False)
    ...
    if auto_open_browser:
        try:
            ...
            try:
                ...
                pass
            logger.warning("Automated token acquisition failed: %s", e)
    ...
    if interactive:
        try:
            ...
            try:
                ...
                pass
            logger.warning("Guided token refresh failed: %s", e)
...
def update_env_file(key, value):
    ...
    try:
        ...
        pass
    logger.info("Updated %s in %s (in-memory env refreshed)", key, env_file)
...
def flask_login_server(api_key, api_secret, auto_run_app=True):
    ...
    cb_host = EnvConfig.get_str("KITE_REDIRECT_HOST", "127.0.0.1")
    cb_port = EnvConfig.get_int("KITE_REDIRECT_PORT", 5000)
    cb_path = EnvConfig.get_str("KITE_REDIRECT_PATH", "success").lstrip("/") or "success"
    ...
    def handle_callback():
        ...
        try:
            logger.info("Callback hit: path=%s args=%s", request.path, dict(request.args))
        except Exception:
            pass
        ...
        try:
            ...
            try:
                ...
                pass
            logger.error("Error exchanging request token: %s", e)
    ...
    timeout = EnvConfig.get_int("KITE_LOGIN_TIMEOUT", 180)  # seconds
    ...
    while not access_token_container['received'] and time.time() - start_time < timeout:
        ...
        if not access_token_container['received']:
            manual_rt = EnvConfig.get_str("KITE_REQUEST_TOKEN", "")
            if manual_rt:
                try:
                    ...
                    try:
                        ...
                        pass
                    logger.error("Error using KITE_REQUEST_TOKEN: %s", e)
    ...
    if access_token_container['received']:
        ...
        print(
            "Tip: Ensure your Redirect URL in Kite developer console EXACTLY matches the "
            "above 'Configured redirect'."
        )
        print(
            "Alternatively, run 'python -m src.tools.token_manager' and choose guided manual "
            "refresh to paste the request_token."
        )
        return None
...
def guided_token_refresh(api_key, api_secret, auto_run_app=True):
    ...
    try:
        ...
        try:
            ...
            pass
        logger.error("Error in guided token refresh: %s", e)
        return None
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description='G6 Platform Token Manager',
        add_help=True,
    )
    parser.add_argument(
        '--no-autorun',
        action='store_true',
        help='Do not automatically run main application',
    )
    parser.add_argument(
        '--provider',
        default=None,
        help='Token provider (env G6_TOKEN_PROVIDER overrides). Default: kite',
    )
    parser.add_argument(
        '--headless',
        action='store_true',
        help='Force headless token acquisition (or env G6_TOKEN_HEADLESS=1)',
    )
    ...
    provider_name = (known_args.provider or EnvConfig.get_str('G6_TOKEN_PROVIDER', '') or 'kite').lower()
    headless = bool(known_args.headless or EnvConfig.get_bool('G6_TOKEN_HEADLESS', False))

    # Lazy import provider registry (optional deps inside providers handled there)
    try:
        if not get_provider:
            raise ImportError("get_provider not available")
    ...
    if not api_key or not api_secret:
        ...
        if 'PYTEST_CURRENT_TEST' in os.environ or EnvConfig.get_bool('G6_TOKEN_HEADLESS', False):
    ...
    if token_valid and not auto_run_app and (headless or getattr(provider, 'name', '') != 'kite'):
        logger.info(
            "Fast-exit: valid token, autorun disabled, headless=%s provider=%s",
            headless,
    ...
    if not token_valid:
        ...
        if new_tok:
            ...
            if provider_validate_token(provider, api_key, new_tok):
                ...
                logger.warning(
                    "Provider returned token but validation failed; falling back to legacy menu for kite provider."
                )
