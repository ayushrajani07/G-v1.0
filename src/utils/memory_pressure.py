...
try:
    ...
    try:
        from src.error_handling import handle_api_error as _handle_api_error_import
    except Exception:
        _handle_api_error_import = None
    if _handle_api_error_import:
        _handle_api_error_import(e, component="utils.memory_pressure.import_psutil")
import logging
import os
import time
import json
...
DEFAULT_TIERS = [
    ...
    PressureTier(
        ...
        2,
        0.80,
        ['shrink_cache', 'reduce_depth', 'skip_greeks', 'slow_cycles'],
    ),
    PressureTier(
        'critical',
        3,
...
class MemoryPressureManager:
    ...
    def evaluate(self):
        ...
        if raw.level < self.current_level:
            ...
            if elapsed < self.recovery_seconds:
                ...
                logger.info(
                    "Memory pressure downgrade allowed after %.0fs stable below thresholds -> %s",
                    elapsed,
                    raw.name,
                )
                self._stable_below_start = None
        ...
        if chosen.level != self.current_level:
            logger.warning(
                "Memory pressure transition %s -> %s (%s) rss=%.1fMB ema%%=%.1f",
                self.current_level,
                chosen.level,
                chosen.name,
                rss_mb,
            ...
            if self.metrics:
                try:
                    ...
                    try:
                        handle_api_error(
                            _e,
                            component="utils.memory_pressure.metrics_inc",
                            context={
                                "action": action,
                                "tier": str(tier.level),
                            },
    ...
    def _maybe_enable_features(self):
        ...
        if (
            not self.greeks_enabled
            and self._greeks_disabled_ts
            and (now - self._greeks_disabled_ts) > self.rollback_cooldown
        ):
            self.greeks_enabled = True
        ...
        if (
            not self.per_option_metrics_enabled
            and self._last_downgrade_ts
            and (now - self._last_downgrade_ts) > (2 * self.rollback_cooldown)
        ):
            self.per_option_metrics_enabled = True
    ...
    def _load_tiers_from_env(self):
        ...
        try:
            ...
            logger.error(
                "Failed parsing G6_MEMORY_PRESSURE_TIERS; using defaults: %s",
                e,
            )
            try:
                handle_data_error(
                    e,
                    component="utils.memory_pressure.load_tiers",
                    context={"env": "G6_MEMORY_PRESSURE_TIERS"},
                )
            except Exception:
                pass
            return DEFAULT_TIERS
