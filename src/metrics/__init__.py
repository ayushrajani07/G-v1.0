...
def _install_deprecation_consolidation():  # pragma: no cover - side-effect instrumentation
	if getattr(_warnings, '_g6_depr_consolidation_installed', False):
		return
	summary = _is_truthy_env('G6_DEPRECATION_SUMMARY') if EnvConfig.get_str('G6_DEPRECATION_SUMMARY', '') else True
	suppress_dupes = _is_truthy_env('G6_DEPRECATION_SUPPRESS_DUPES') if EnvConfig.get_str('G6_DEPRECATION_SUPPRESS_DUPES', '') else True
...
_prev_ctx = EnvConfig.get_str('G6_METRICS_IMPORT_CONTEXT', '')
try:
...
def get_metrics_singleton():
	...
	# Allow tests to force a brand new registry instance, and also ensure
	# snapshot runs operate on a fresh registry so baseline maps are complete
	...
	if existing is not None:
		...
		try:
			if (getattr(existing, '_metrics_introspection', None) is None and (
				_is_truthy_env('G6_METRICS_EAGER_INTROSPECTION') or EnvConfig.get_str('G6_METRICS_INTROSPECTION_DUMP','')
			)):
		...
		try:
			_snap = EnvConfig.get_str('G6_CARDINALITY_SNAPSHOT','').strip()
			if _snap:
		...
		try:
			...
			if '_G6_METRICS_SUMMARY_EMITTED' not in _m.__dict__:
				import logging as __logging
				...
				__logging.getLogger('src.metrics.metrics').info(
					...
					fam_count, len(getattr(_m, 'ALWAYS_ON_GROUPS', set())), profile_total, int(_is_truthy_env('G6_METRICS_STRICT_EXCEPTIONS'))
				)
				_m.__dict__['_G6_METRICS_SUMMARY_EMITTED'] = True
				try:
		...
		try:  # pragma: no cover - defensive wrapper
			...
			if not already:
				...
				want_introspection_dump = bool(EnvConfig.get_str('G6_METRICS_INTROSPECTION_DUMP','').strip())
				want_init_trace_dump = bool(EnvConfig.get_str('G6_METRICS_INIT_TRACE_DUMP','').strip())
				if suppress:
					logger.info("metrics.dumps.suppressed reason=G6_METRICS_SUPPRESS_AUTO_DUMPS env=%s introspection_dump=%s init_trace_dump=%s", EnvConfig.get_str('G6_METRICS_SUPPRESS_AUTO_DUMPS',''), EnvConfig.get_str('G6_METRICS_INTROSPECTION_DUMP',''), EnvConfig.get_str('G6_METRICS_INIT_TRACE_DUMP',''))
	...
	try:
		...
		if getattr(reg, '_metrics_introspection', None) is None:
			if _is_truthy_env('G6_METRICS_EAGER_INTROSPECTION') or EnvConfig.get_str('G6_METRICS_INTROSPECTION_DUMP',''):
				try:
					from .introspection import build_introspection_inventory as _bii
	...
	try:
		...
		_snap = _EC.get_str('G6_CARDINALITY_SNAPSHOT','').strip()
		if _snap:
			import os as __os
			_need = (not __os.path.exists(_snap)) or (__os.path.getsize(_snap) == 0)
			if _need:
...
_force_facade_registry = (
	...
	or bool(EnvConfig.get_str('G6_CARDINALITY_SNAPSHOT','') or EnvConfig.get_str('G6_CARDINALITY_BASELINE',''))
)
...
try:  # pragma: no cover - guard detection
	if not _disable_eager:
		...
		_explicit_disable_val = EnvConfig.get_str('G6_METRICS_EAGER_DISABLE','')
