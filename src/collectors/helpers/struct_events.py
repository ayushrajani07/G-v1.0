"""struct_events.py — Structured event emitters for G6 collectors.

Phase 10 standardization: structured logging layer decoupled from legacy tables.
Fine-grained suppression via G6_STRUCT_EVENTS_SUPPRESS='event1,event2'.
"""
...
_STRUCT_DISABLED = EnvConfig.get_bool('G6_DISABLE_STRUCT_EVENTS', False)

# Fine-grained suppression (comma/space separated list of event names)
_STRUCT_SUPPRESS: set[str] = set()
try:
    _raw_sup = EnvConfig.get_str('G6_STRUCT_EVENTS_SUPPRESS', '')
    if _raw_sup:
...
_STRUCT_COMPACT = EnvConfig.get_bool('G6_STRUCT_COMPACT', False)
import os
import time
from typing import Any
