...
try:
    ...
    emit_strike_depth_adjustment = None  # type: ignore
...
def adaptive_strike_retry(ctx: AdaptiveCtxLike | Any, index_symbol: str, expiry_rec: ExpiryRecordLike, expiry_rule: str) -> None:
    ...
    if not hasattr(ctx, '_adaptive_retry_cache'):
        ctx._adaptive_retry_cache = {
            'disable': EnvConfig.get_bool('G6_DISABLE_ADAPTIVE_STRIKE_RETRY', False),
            'strike_ok_raw': EnvConfig.get_str('G6_STRIKE_COVERAGE_OK'),
            'max_itm_raw': EnvConfig.get_str('G6_ADAPTIVE_STRIKE_MAX_ITM'),
            'max_otm_raw': EnvConfig.get_str('G6_ADAPTIVE_STRIKE_MAX_OTM'),
            'step_raw': EnvConfig.get_str('G6_ADAPTIVE_STRIKE_STEP'),
        }
...
def adaptive_contraction(ctx: AdaptiveCtxLike | Any, index_symbol: str, expiry_rec: ExpiryRecordLike, expiry_rule: str) -> None:
    ...
    ok_cycles = EnvConfig.get_int('G6_CONTRACT_OK_CYCLES', 5)
    cooldown = EnvConfig.get_int('G6_CONTRACT_COOLDOWN', 3)
    step_dec = EnvConfig.get_int('G6_CONTRACT_STEP', 2)
    idx_cfg = (getattr(ctx, 'index_params', {}) or {}).get(index_symbol, {})
    ...
    if cur_cycle_num is None:
        interval_s = EnvConfig.get_int('G6_CYCLE_INTERVAL', 60)
