...
try:
    from src.utils.status_reader import get_status_reader as _get_status_reader_import
except ImportError:
    _get_status_reader_import = None  # type: ignore
try:
    from src.utils.csv_cache import read_json_cached as _read_json_cached_import
except ImportError:
    _read_json_cached_import = None  # type: ignore
try:
    from src.utils.metrics_utils import init_metrics as _init_metrics_import
except ImportError:
    _init_metrics_import = None  # type: ignore
...
def _bootstrap(config_path: str) -> tuple[RuntimeContext, Callable[[], None]]:  # type: ignore[name-defined]
    ...
    # Normalize stop function to a callable to satisfy type contract
    if not callable(metrics_stop):
        def _noop() -> None:
            pass
        metrics_stop_fn: Callable[[], None] = _noop
    else:
        metrics_stop_fn = cast(Callable[[], None], metrics_stop)
    ...
    try:
        for i in range(cycles if cycles > 0 else 1_000_000):
            ...
            if not args.no_view:
                ...
                try:
                    try:
                        if _get_status_reader_import:
                            reader = _get_status_reader_import(args.status_file)
                            data = reader.get_raw_status()
                        else:
                            raise ImportError("status_reader not available")
                    except Exception:
                        from pathlib import Path as _Path

                        if _read_json_cached_import:
                            data = _read_json_cached_import(_Path(args.status_file))
                        else:
                            data = None
...
def cmd_view_status(args: argparse.Namespace) -> int:
    ...
    try:
        while True:
            if path.exists():
                try:
                    if _get_status_reader_import is not None:
                        reader = _get_status_reader_import(str(path))
                        data = reader.get_raw_status()
                    elif _read_json_cached_import:
                        data = _read_json_cached_import(path)
                    else:
                        data = None
...
def cmd_simulate_status(args: argparse.Namespace) -> int:
    ...
    if args.start_metrics_server:
        try:
            if _init_metrics_import:
                _init_metrics_import(port=9108)
                print('[simulate-status] metrics server started on 127.0.0.1:9108')
            else:
                print('[simulate-status] init_metrics not available; skipping metrics server.')
        except Exception as e:  # noqa: BLE001
