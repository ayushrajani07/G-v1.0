...
class InfluxBufferManager:
    ...
    def _send(self, points: list[Any]) -> None:
        ...
        for attempt in range(self._max_retries + 1):
            ok, _err = self._try_send_once(points)
            if ok:
                return
    ...
    def _try_send_once(self, points: list[Any]) -> tuple[bool, Exception | None]:
        """Attempt a single send; returns (ok, error). Helper avoids try/except in retry loop."""
        try:
            if points:
