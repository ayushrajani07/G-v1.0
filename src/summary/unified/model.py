...
def assemble_model_snapshot(
    *,
    runtime_status: dict[str, Any] | None,
    panels_dir: str | None = None,
    include_panels: bool = True,
    in_memory_panels: dict[str, Any] | None = None,
) -> tuple[UnifiedStatusSnapshot, dict[str, Any]]:
    ...
    try:
        ...
        if include_panels and in_memory_panels:
            try:
                if isinstance(in_memory_panels.get('provider'), dict):
                    panel_provider = in_memory_panels.get('provider')
                if isinstance(in_memory_panels.get('resources'), dict):
                    panel_resources = in_memory_panels.get('resources')
                if isinstance(in_memory_panels.get('loop'), dict):
                    panel_loop = in_memory_panels.get('loop')
                if isinstance(in_memory_panels.get('indices'), dict):
                    panel_indices = in_memory_panels.get('indices')
                if isinstance(in_memory_panels.get('adaptive_alerts'), dict):
                    panel_adaptive_alerts = in_memory_panels.get('adaptive_alerts')
        ...
        try:
            prov = None
            if isinstance(panel_provider, dict):
                prov = panel_provider
            else:
                st_prov = status.get('provider') if isinstance(status.get('provider'), dict) else None
            ...
            if isinstance(prov, dict):
                model.provider = {
                    ...
                    'auth_valid': (
                        (prov.get('auth') or {}).get('valid')
                        if isinstance(prov.get('auth'), dict) else None
                    ),
                    'auth_expiry': (
                        (prov.get('auth') or {}).get('expiry')
                        if isinstance(prov.get('auth'), dict) else None
                    ),
                }
        ...
        try:
            res_obj = None
            if isinstance(panel_resources, dict):
                res_obj = panel_resources
            else:
                st_res = status.get('resources') if isinstance(status.get('resources'), dict) else None
        ...
        try:
            source_indices = {}
            if isinstance(panel_indices, dict):
                source_indices = panel_indices
            else:
                st_idx = status.get('indices_detail') if isinstance(status.get('indices_detail'), dict) else {}
                if isinstance(st_idx, dict):
                    source_indices = st_idx
            out: list[IndexEntry] = []
        ...
        # --- DQ Classification ---
        try:
            warn_thr = EnvConfig.get_float('G6_DQ_WARN_THRESHOLD', 85.0)
            err_thr = EnvConfig.get_float('G6_DQ_ERROR_THRESHOLD', 70.0)
