...
try:
    ...
    # Optional at runtime; type as Any to avoid narrow stubs when missing
    handle_data_collection_error = None  # noqa: N816
    handle_data_error = None  # noqa: N816

try:
    import tracemalloc
except Exception:  # pragma: no cover
    from typing import Any as _Any
    tracemalloc: _Any | None = None


class MemoryTracer:
    def __init__(self, topn: int | None = None, snapshot_dir: str | None = None, write_snapshots: bool | None = None):
        if is_truthy_env is None:
            self.enabled = False
        else:
        ...
        self.topn = int(
            os.environ.get(
                'G6_TRACEMALLOC_TOPN',
                str(topn if topn is not None else 10),
            )
            or '10'
        )
        self.snapshot_dir = os.environ.get(
            'G6_TRACEMALLOC_SNAPSHOT_DIR',
            snapshot_dir or 'logs/mem',
        )
        if is_truthy_env is None:
            ws = False
        else:
            ws = is_truthy_env('G6_TRACEMALLOC_WRITE_SNAPSHOTS')
        self.write_snapshots = (
            ws
            if write_snapshots is None
            else bool(write_snapshots)
        )
        self._started = False
            ...
            try:
                ...
                try:
                    if handle_data_error:
                        handle_data_error(
                            Exception("tracemalloc_start_failed"),
                            component="utils.memory_trace",
                            context={"op": "start"},
                        )
                except Exception:
                    pass
                    ...
                    try:
                        if handle_data_collection_error:
                            handle_data_collection_error(
                                e,
                                component="utils.memory_trace",
                                data_type="tracemalloc_snapshot",
                                context={"dir": self.snapshot_dir},
                            )
                    except Exception:
                        pass
        except Exception as e:
