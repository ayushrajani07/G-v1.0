...
Goals:
- Provide standardized logic for mapping a tradingsymbol to an index root
    (e.g. NIFTY, BANKNIFTY, FINNIFTY, MIDCPNIFTY, SENSEX, etc.).
- Avoid substring collisions (e.g. NIFTY inside FINNIFTY) by using prefix +
    boundary heuristics.
- Allow future extension to new exchanges or naming conventions without
    touching provider logic.
- Support multiple match modes via env var G6_SYMBOL_MATCH_MODE:
    strict (default): prefix match + next token begins with a digit (date) or
    recognized month code.
...
_DEF_MODE = EnvConfig.get_str("G6_SYMBOL_MATCH_MODE", "strict").strip().lower()
...
def detect_root(tradingsymbol: str) -> str | None:
    ...
    Longest root wins to prevent partial classification (e.g., MIDCPNIFTY
    before NIFTY).
    """
