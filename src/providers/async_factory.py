...
# Optional imports for late import elimination (Batch 33)
try:
...
def create_async_provider(provider_type: str, config: dict[str, Any] | None = None):
    ...
    max_workers = EnvConfig.get_int('G6_PARALLEL_MAX_WORKERS', cfg.get('max_workers', 8))
    cps = EnvConfig.get_float('G6_KITE_RATE_LIMIT_CPS', cfg.get('rate_cps', 0))
    burst = EnvConfig.get_int('G6_KITE_RATE_LIMIT_BURST', cfg.get('rate_burst', 0))

    if ptype in ("kite", "zerodha", "kiteconnect"):
        if not kite_provider_factory:
