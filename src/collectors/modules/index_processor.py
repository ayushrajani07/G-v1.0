...
# Module imports (extracted late imports to module top)
...
from src.collectors.modules.strike_depth import compute_strike_universe
...
from src.collectors.helpers.status_reducer import aggregate_cycle_status, compute_expiry_status
from src.logstream.formatter import format_index

# Optional imports for late import elimination (Batch 32)
try:
    from src.collectors.unified_collectors import _build_expiry_map  # type: ignore[attr-defined]
except ImportError:
    _build_expiry_map = None  # type: ignore[assignment]
...
def process_index(
    ...
    *,
    ...
    try:
        ...
        logger.error("Unexpected ATM strike error %s: %s", index_symbol, e)
        atm_strike = 0
    ...
    try:
        ...
        if not precomputed_strikes: logger.warning("Precomputed strike list empty for %s; atm=%s", index_symbol, atm_strike)
        else: logger.debug("Precomputed %s strikes for %s: %s", len(precomputed_strikes), index_symbol, precomputed_strikes)
        ...
        if not _env_bool('G6_DISABLE_EXPIRY_MAP', False):
            ...
            if callable(raw_universe_fetch):
                ...
                if not _build_expiry_map:
                    logger.debug('_build_expiry_map unavailable, skipping expiry map build')
                else:
                    try:
