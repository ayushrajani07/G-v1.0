...
class ResolvePhase:
    ...
    def run(self, ctx: Any, settings: Any, state: ExpiryState) -> ExpiryState:
        try:
            expiry_date = resolve_expiry(state.index_symbol, state.expiry_rule, ctx.providers, getattr(ctx, 'metrics', None), getattr(ctx, 'concise_mode', False))
    ...
    def run(self, ctx: Any, settings: Any, state: ExpiryState) -> ExpiryState:
        ...
        try:
            if getattr(ctx, 'expiry_universe_map', None) and state.expiry_date in ctx.expiry_universe_map:
                ...
                instruments = fetch_option_instruments(state.index_symbol, state.expiry_rule, state.expiry_date, state.strikes, ctx.providers, getattr(ctx, 'metrics', None))
...
class EnrichPhase:
    ...
    def run(self, ctx: Any, settings: Any, state: ExpiryState) -> ExpiryState:
        ...
        if not state.fetched_instruments or state.expiry_date is None or not state.strikes:
            return state
        try:
            enriched = enrich_quotes(state.index_symbol, state.expiry_rule, state.expiry_date, state.fetched_instruments, ctx.providers, getattr(ctx, 'metrics', None))
