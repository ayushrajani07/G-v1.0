...
class CircuitBreaker:
    ...
    def call(self, func: Callable, *args, **kwargs) -> Any:
        ...
        with self._lock:
            if self.state == CircuitState.OPEN:
                if time.time() - self.last_failure_time >= self.reset_timeout:
                    self.logger.info("Circuit %s half-opening after timeout", self.name)
        ...
        try:
            ...
            with self._lock:
                if self.state == CircuitState.HALF_OPEN:
                    self.logger.info("Circuit %s closing after successful half-open call", self.name)
                    self.state = CircuitState.CLOSED
                    self.failure_count = 0
                    self.half_open_calls = 0
            ...
            with self._lock:
                ...
                if self.state == CircuitState.HALF_OPEN:
                    self.logger.warning("Circuit %s reopening after failed half-open call: %s", self.name, e)
                    try:
                    ...
                    if self.failure_count >= self.failure_threshold:
                        self.logger.warning("Circuit %s opening after %s failures", self.name, self.failure_count)
                        try:
                            if get_error_handler and ErrorCategory and ErrorSeverity:
                                get_error_handler().handle_error(
                                    exception=e,
...
if __name__ == "__main__":
    ...
    def _attempt(i: int, *, should_fail: bool) -> None:
        try:
            res = api_call(should_fail=should_fail)
            print(f"Call {i}: {res}")
    ...
    try:
        for i in range(5):
            _attempt(i, should_fail=True)

        print("Waiting for circuit to half-open...")
        time.sleep(6)

        try:
            res = api_call(should_fail=False)
            print(f"After wait: {res}")
        except Exception as exc:  # noqa: BLE001 - demo code
            print(f"After wait failed: {exc}")
