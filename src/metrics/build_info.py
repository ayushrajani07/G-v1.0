...
def register_build_info(metrics: Any, version: str | None = None, git_commit: str | None = None, config_hash: str | None = None) -> None:
    ...
    try:
        ...
        if gauge is not None:
            v = version or EnvConfig.get_str('G6_BUILD_VERSION', 'unknown')
            gc = git_commit or EnvConfig.get_str('G6_BUILD_COMMIT', 'unknown')
            ch = config_hash or EnvConfig.get_str('G6_BUILD_CONFIG_HASH', 'unknown')
            relabeled = False
            try:
