...
class AdaptiveConfig:
    exit_backlog_ratio: float = field(default_factory=lambda: EnvConfig.get_float('G6_ADAPT_EXIT_BACKLOG_RATIO', 0.4))
    exit_window_seconds: float = field(default_factory=lambda: EnvConfig.get_float('G6_ADAPT_EXIT_WINDOW_SECONDS', 5.0))
    latency_budget_ms: float = field(default_factory=lambda: EnvConfig.get_float('G6_ADAPT_LAT_BUDGET_MS', 50.0))
    reentry_cooldown_seconds: float = field(default_factory=lambda: EnvConfig.get_float('G6_ADAPT_REENTRY_COOLDOWN_SECONDS', 10.0))
    min_samples: int = field(default_factory=lambda: EnvConfig.get_int('G6_ADAPT_MIN_SAMPLES', 10))
