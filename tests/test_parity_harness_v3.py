...
try:  # pragma: no cover
    from src.parity import signature as _sig_import  # type: ignore
except ImportError:
    _sig_import = None  # type: ignore
...
def _flatten_groups(snap_summary: dict, include: bool) -> Dict[str, Any]:
    ...
    """
    if _sig_import:  # pragma: no cover - defensive import variability
        build = getattr(_sig_import, 'build_parity_signature', None)
