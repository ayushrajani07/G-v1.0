...
def _http_get(url: str, timeout: float = 1.5, insecure: bool = True) -> tuple[int, bytes]:
    ...
    try:
        ...
        if hasattr(e, 'code'):
            return int(e.code), b""  # type: ignore[attr-defined]
        return -1, b""
...
def _detect_prometheus(hosts=("localhost","127.0.0.1"), ports=range(9090, 9101)) -> ServiceStatus:
    ...
    env_url = EnvConfig.get_str("G6_PROM_URL", "")
    if env_url:
        try:
...
def _attempt_ps_autostart(timeout_sec: float = 45.0) -> None:
    ...
    try:
        ...
        if EnvConfig.get_str("G6_INFLUXD_EXE", ""):
            print(f"[auto-resolve] Using G6_INFLUXD_EXE={EnvConfig.get_str('G6_INFLUXD_EXE', '')}")
