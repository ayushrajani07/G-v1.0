...
try:  # rate limiter optional (Phase 1)
    from .rate_limit import RateLimitedError, build_default_rate_limiter  # type: ignore[assignment]
except Exception:  # pragma: no cover
        ...
        try:
            ...
            if isinstance(item, (tuple, list)):
                ...
                if len(item) == 1:
                    ...
                    continue
            logger.debug("Skipping malformed instrument entry: %s", item)
        except Exception:
...
def get_ltp(provider: ProviderLike | Any, instruments: Iterable[InstrumentLike]) -> dict[str, Any]:
    ...
    try:
            ...
            if formatted:
                    ...
                    if EnvConfig.get_bool('G6_KITE_LIMITER', False):
                        ...
                        if limiter is None:
                            try:
                                ...
                                provider._g6_ltp_rate_limiter = limiter
                            except Exception:
                                limiter = None
                ...
                try:
                    ...
                    try:
                        ...
                        logger.debug("LTP response unusable (%s); switching to synthetic values", ltp_quality_err)
                except Exception as retry_err:
                    # Record rate limit errors for cooldown tracking
                    if limiter is not None:
                        try:
                            msg = str(retry_err).lower()
        ...
        if _is_auth_error(e) or str(e) == 'kite_auth_failed':
            ...
            logger.debug("LTP real fetch failed, using synthetic: %s", e)
        try:
...
def get_quote(provider: ProviderLike | Any, instruments: Iterable[InstrumentLike]) -> dict[str, QuoteTD]:
    ...
    try:
        ...
        if _is_auth_error(e) or str(e) == 'kite_auth_failed':
            ...
            if getattr(provider, '_rl_quote_fallback', lambda: True)():
                logger.debug("Quote real fetch failed, falling back to LTP: %s", e)
            # If failure is due to provider rate limiting, optionally skip real LTP fetch
            try:
                msg = str(e).lower()
                if EnvConfig.get_bool('G6_QUOTE_SKIP_LTP_ON_RATELIMIT', True) and (
    ...
    if 'skip_real_ltp' in locals() and skip_real_ltp:
        # Build synthetic LTP locally to avoid further stressing provider during rate-limit windows
        ltp_data = _synthetic_ltp(provider, instruments)
    else:
