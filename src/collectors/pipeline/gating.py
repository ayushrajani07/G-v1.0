...
def load_config_from_env() -> GatingConfig:
    ...
    mode = EnvConfig.get_str("G6_SHADOW_GATE_MODE", "off").strip().lower()
    raw_list = EnvConfig.get_str('G6_SHADOW_CANARY_INDICES', '').strip()
    ...
    extra_protected_raw = EnvConfig.get_str('G6_SHADOW_PROTECTED_FIELDS', '').strip()
...
def decide(index: str, rule: str, meta: Mapping[str, Any], *, config: GatingConfig | None = None, store: WindowStore | None = None) -> dict[str, Any]:
    ...
    if window_size < config.min_samples:
        ...
        if EnvConfig.get_bool('G6_SHADOW_FORCE_DEMOTE', False):
    ...
    if mode == 'promote':
        ...
        force_demote = EnvConfig.get_bool('G6_SHADOW_FORCE_DEMOTE', False)
        if force_demote:
            decision['reason'] = 'forced_demote'
            decision['promote'] = False
            decision['canary'] = False
        ...
        if ratio >= config.parity_target and ok_streak >= config.ok_hysteresis:
            ...
            if EnvConfig.get_bool('G6_SHADOW_AUTHORITATIVE', False):
