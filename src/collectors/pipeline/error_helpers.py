...
# Optional imports
try:
    from src.metrics import get_metrics  # runtime optional import
except ImportError:
...
def add_phase_error(state: ExpiryState, phase: str, classification: str, message: str, *, detail: str | None=None, attempt: int=1, token: str | None=None, extra: dict | None=None) -> None:
    ...
    try:
        ...
        patterns = EnvConfig.get_str('G6_PIPELINE_REDACT_PATTERNS', '')
        replacement = EnvConfig.get_str('G6_PIPELINE_REDACT_REPLACEMENT', '***')
        if patterns:
            try:
