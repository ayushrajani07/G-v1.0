...
├── metrics/
│   └── metrics.py            # Consolidated metrics (Prometheus text exposition format)
├── analytics/
...
- **Metrics endpoint (Prometheus text format)**: `http://localhost:9108/metrics` (no Prometheus server required)
...
- **Monitoring**: Comprehensive metrics endpoint (Prometheus exposition format)
...
- The metrics endpoint still exposes `g6_index_options_processed{index=...}` (last cycle) and `g6_index_options_processed_total{index=...}` (monotonic cumulative).
