...
Docs are being migrated into [docs/README.md](docs/README.md). Start with:

- Quickstart: [docs/getting-started/QUICKSTART.md](docs/getting-started/QUICKSTART.md)
- Architecture snapshot: [docs/architecture/ARCHITECTURE_SNAPSHOT.md](docs/architecture/ARCHITECTURE_SNAPSHOT.md)
- Operations index: [docs/operations/README_INDEX.md](docs/operations/README_INDEX.md)

High‑throughput, modular options market data collection & analytics platform for Indian indices (NIFTY, BANKNIFTY, FINNIFTY, SENSEX, …). It performs minute‑level (configurable) collection cycles, computes derived analytics (IV, Greeks, PCR, breadth), writes durable snapshots (CSV + optional InfluxDB), and exposes rich observability (metrics endpoint in Prometheus text format, summary panels, JSON status manifests).
...
- Extensible, group‑gated metrics (Prometheus exposition format) & integrity‑verified panel artifacts
...
If you use VS Code tasks, prefer wiring them to the canonical script launcher:
* `./scripts/start.ps1 -Mode panels-flow` (simulator + panels + summary)
* `./scripts/start.ps1 -Mode summary` (interactive summary)
* `./scripts/start.ps1 -Mode obs` (Grafana + Web API)
...
- **Local Installation Paths**: [LOCAL_PATHS.md](LOCAL_PATHS.md) – Reference for Grafana, metrics endpoints, Python paths and service endpoints
...
| Web API | `src/web/api/`, `src/web/dashboard/` | JSON API for Grafana + diagnostics (HTML removed) | [C] | Keep surface minimal; debug routes gated |
...
1. **Configuration Loading Consolidation** _(Oct 25, 2025)_
...
For detailed status and methodology, see `INEFFICIENCIES_REPORT.md`.
...
Metrics endpoint (Prometheus exposition format) embedded. Metrics grouped & gated to control cardinality.
...
3. Grafana dashboards over JSON Web API (recommended; Infinity queries)
4. Metrics endpoint in Prometheus text format (optional to scrape)
...
5. (Optional) If you run Prometheus, it can ingest recording/alert rules; Grafana dashboards can then use the derived metrics.
