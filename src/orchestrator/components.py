...
try:
    ...
    m_api_calls_total_labels = None  # type: ignore
    m_quote_enriched_total_labels = None  # type: ignore
try:
    from src.storage.csv_sink import CsvSink as _CsvSinkImport  # type: ignore
except ImportError:
    _CsvSinkImport = None  # type: ignore
try:
    from src.providers.factory import create_provider as _CreateProviderImport  # type: ignore
except ImportError:
    _CreateProviderImport = None  # type: ignore
try:
    from src.providers.composite_provider import CompositeProvider as _CompositeProviderImport
except ImportError:
    _CompositeProviderImport = None  # type: ignore
...
try:  # optional imports preserved; fallbacks for early stages
    ...
    if not _CompositeProviderImport:
        _CompositeProvider = None
    else:
        _CompositeProvider = _CompositeProviderImport
    ...
    logger.warning(
        (
            "[fallback Providers] Import failure activating degraded "
            "provider facade: %s"
        ),
        _providers_import_err,
    )
    class _ProvidersFallback:
        ...
        def get_index_data(self, index_symbol):  # noqa: D401
            ...
            if not prov or not hasattr(prov, 'get_ltp'):
                self.logger.warning(
                    (
                        "[fallback Providers] No primary provider LTP available; "
                        "returning 0 price for %s"
                    ),
                    index_symbol,
                )
                return 0, {}
            try:
                if index_symbol == 'NIFTY':
                    ltp_obj = prov.get_ltp([('NSE', 'NIFTY 50')])
                else:
                    ltp_obj = prov.get_ltp([(index_symbol,)])  # permissive
        ...
        def resolve_expiry(self, index_symbol, expiry_rule):  # noqa: D401
            ...
            try:
                ...
                if (
                    isinstance(expiry_rule, str)
                    and len(expiry_rule) == 10
                    and expiry_rule[4] == '-'
                    and expiry_rule[7] == '-'
                ):
                    y, m, d = expiry_rule.split('-')
                    return _d.date(int(y), int(m), int(d))
                today = _d.date.today()
            ...
            try:
                ...
                def _build_pair(k: int) -> tuple[dict, dict]:
                    ...
                    pe.update({
                        ...
                        'tradingsymbol': f"{index_symbol}{exp_str}{k}PE",
                        'instrument_type': 'PE',
                    })
                    return ce, pe
                for s in strikes:
        ...
        def enrich_with_quotes(self, instruments):  # noqa: D401
            ...
            try:  # switch to generated metric helpers (lazy registration)
                if not m_api_calls_total_labels or not m_quote_enriched_total_labels:
                    raise ImportError("metrics.generated not available")
                ...
                if enriched_count:
                    ...
                    _safe_inc(lbl_q, enriched_count)
    ...
    def _create_provider_noop(*a, **k):
        return None
    create_provider_fn = _create_provider_noop
    CsvSinkT = object
...
try:
    if 'CsvSinkT' in globals() and getattr(CsvSinkT, '__name__', '') == 'object':
        if _CsvSinkImport:
            CsvSinkT = _CsvSinkImport
        else:
            raise ImportError("CsvSink not available")
    ...
    if 'create_provider_fn' in globals() and getattr(create_provider_fn, '__name__', '') == '_create_provider_noop':
        try:
            if _CreateProviderImport:
    ...
    _CSV_REIMPORT_ERR_MSG = f"CsvSink import failed: {_csv_reimport_err}"
    class _NullCsvSink:
        def __init__(self, base_dir: str, _reason: str | None = None):  # mimic signature
            self.base_dir = base_dir
            self.logger = logger
            self.metrics = None
            if _reason is None:
                _reason = _CSV_REIMPORT_ERR_MSG
            logger.warning("Using _NullCsvSink placeholder (%s)", _reason)
...
def init_providers(config) -> Any:
    if EnvConfig.get_bool('G6_USE_MOCK_PROVIDER', False):
        import random
        class MockProvider:  # minimal facade-compatible surface
            def __init__(self):
                ...
                try:
                    if get_index_meta:
                        self._bases = {
                            s: float(get_index_meta(s).synthetic_atm)
    ...
    try:
        if is_truthy_env('G6_COMPOSITE_PROVIDER'):
            ...
            sec_path = EnvConfig.get_str('G6_SECONDARY_PROVIDER_PATH', '')
            if sec_path:
                try:
