...
def run_enhanced_collectors(index_params: dict[str, Any], providers, csv_sink, influx_sink, metrics, *_, **kw):  # signature kept lax
    ...
    snapshot_intent = bool(kw.get('return_snapshots')) or EnvConfig.get_bool('G6_RETURN_SNAPSHOTS', False)
    if EnvConfig.get_bool('G6_ENHANCED_SNAPSHOT_MODE', False) or snapshot_intent:
