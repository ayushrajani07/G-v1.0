# PROMETHEUS REMOVED: PrometheusPort parameter no longer needed (Prometheus not used)
param(
  ...
  foreach ($procId in ($Pids | Sort-Object -Unique)) {
    try {
      if (Get-Process -Id $procId -ErrorAction SilentlyContinue) {
        ...
        # Try normal Stop-Process first
        try { Stop-Process -Id $procId -Force -ErrorAction SilentlyContinue } catch {}
        Start-Sleep -Milliseconds 150
        # If still present, escalate to taskkill to terminate the entire tree (/T)
        if (Get-Process -Id $procId -ErrorAction SilentlyContinue) {
...
# PROMETHEUS REMOVED: No longer stopping Prometheus (not used by dashboards)
# Stop-ByPort -Port $PrometheusPort -Label 'prometheus'
Stop-ByPort -Port $InfluxPort -Label 'influxdb'

if ($Aggressive) {
  Write-Host 'Aggressive mode: attempting name-based termination for grafana-server, influxd' -ForegroundColor Yellow
  foreach ($name in @('grafana-server','grafana','influxd')) {
    try { Get-Process -Name $name -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue } catch {}
  }
}

# Final status summary of monitored ports (PROMETHEUS REMOVED)
foreach ($p in @(
