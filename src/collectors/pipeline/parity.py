...
def compute_parity_score(legacy: dict[str, Any] | None, pipeline: dict[str, Any] | None, *, weights: dict[str, float] | None = None) -> ParityResult:
    extended = EnvConfig.get_bool('G6_PARITY_EXTENDED', False)
    enable_shape = EnvConfig.get_bool('G6_PARITY_STRIKE_SHAPE', False)
    enable_cov_var = EnvConfig.get_bool('G6_PARITY_STRIKE_COV_VAR', False)
    ...
    try:
        ...
        raw_weights = EnvConfig.get_str('G6_PARITY_ALERT_WEIGHTS', '')
    ...
    try:
        override_raw = EnvConfig.get_str('G6_PARITY_COMPONENT_WEIGHTS', '')
        if override_raw:
