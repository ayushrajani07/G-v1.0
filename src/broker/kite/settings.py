...
def load_settings() -> Settings:
    ...
    raw_concise = EnvConfig.get_str("G6_CONCISE_LOGS", "")
    if raw_concise == "":
        concise = True
    else:
        concise = raw_concise.lower() not in ("0","false","no","off")
    
    kite_timeout_str = EnvConfig.get_str("G6_KITE_TIMEOUT", "")
    if not kite_timeout_str:
        kite_timeout_str = EnvConfig.get_str("G6_KITE_TIMEOUT_SEC", "4.0")
    kite_timeout = float(kite_timeout_str or "4.0")
    
    # Instruments can legitimately take longer; allow a separate, higher default
    kite_instr_timeout_str = EnvConfig.get_str("G6_KITE_INSTRUMENTS_TIMEOUT_SEC", "")
    if not kite_instr_timeout_str:
        kite_instr_timeout_str = EnvConfig.get_str("G6_KITE_TIMEOUT_SEC", "")
    kite_instr_timeout = float(kite_instr_timeout_str or "8.0")
    
    throttle_ms = EnvConfig.get_int("G6_KITE_THROTTLE_MS", 0)
    ttl = EnvConfig.get_float('G6_INSTRUMENT_CACHE_TTL', 600.0)
    
    lean_mode = EnvConfig.get_bool('G6_LEAN_MODE', False)
    debug_short_ttl = EnvConfig.get_bool('G6_DEBUG_SHORT_TTL', False)
    ...
    quiet_mode = EnvConfig.get_str('G6_QUIET_MODE', '') == '1'
    quiet_allow_trace = EnvConfig.get_bool('G6_QUIET_ALLOW_TRACE', False)
    disable_prefilter = EnvConfig.get_bool('G6_DISABLE_PREFILTER', False)
    enable_nearest = EnvConfig.get_bool('G6_ENABLE_NEAREST_EXPIRY_FALLBACK', True)
    enable_backward = EnvConfig.get_bool('G6_ENABLE_BACKWARD_EXPIRY_FALLBACK', True)
    return Settings(
        concise=concise,
