...
def test_move_exporter_writes_header_and_row(tmp_path: Path, monkeypatch):
    ...
    # Dummy classifier/regressor artifacts (now top-level for pickling)
    clf_art = {"model": DummyClassifier(strategy="prior"), "features": [
        "index_price", "minutes_to_expiry", "ce_iv", "pe_iv", "tp", "tp_net_change", "tp_day_change"
    ]}
    reg_art = {"model": DummyRegressor(strategy="mean"), "features": clf_art["features"], "mean_magnitude": 4.0}
    ...
    # Act: import exporter and run single iteration with --once flag in-process
    import sys
    from scripts.ml import move_predict_exporter as mpe  # type: ignore
    sys.argv = [
        "move_predict_exporter.py",
        "--config", str(cfg),
        "--classifier-artifact", str(clf_path),
        "--regressor-artifact", str(reg_path),
        "--index", "TEST",
        "--horizon", "1",
        "--interval", "1",
