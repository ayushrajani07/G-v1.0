...
try:
    from src.utils.metrics_adapter import get_metrics_adapter as _get_metrics_adapter_import
except ImportError:
    _get_metrics_adapter_import = None  # type: ignore


def test_metrics_adapter_safe_import():
    try:
        if not _get_metrics_adapter_import:
            pytest.skip("metrics_adapter not available")
...
def test_metrics_adapter_safe_getters():
    ...
    ma = _get_metrics_adapter_import()
