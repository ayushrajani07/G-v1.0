...
def merge_panel_diff(base: Any, delta: Any) -> Any:
    ...
    - For dicts:
        ...
        * Otherwise recursively merge.
        - For lists:
                ...
                * If both lists of dicts and lengths differ modestly (< 50 items),
                    attempt index-wise merge up to min length then append remaining
                    tail from longer.
                * Else replace.
    ...
    if isinstance(base, list):
        ...
        if (all(isinstance(x, Mapping) for x in base)
                and all(isinstance(x, Mapping) for x in delta)
                and len(base) < 200 and len(delta) < 200):
            # Merge index-wise for overlap, then append remaining tail from delta
            limit = min(len(base), len(delta))
            merged = [merge_panel_diff(base[i], delta[i]) for i in range(limit)]
            if len(delta) > len(base):
                merged.extend(_clone(x) for x in delta[len(base):])
