...
class HealthMonitor:
        ...
        while self.running:
            try:
                ...
                try:
                    ...
                    pass
                self.logger.error("Error in health monitoring: %s", e)
        ...
        for name, data in self.components.items():
            try:
                ...
                try:
                    ...
                    pass
                self.logger.error("Error checking component %s: %s", name, e)
                self.components[name]['last_check'] = datetime.datetime.now()  # local-ok
        ...
        for check in self.health_checks:
            try:
                ...
                try:
                    ...
                    pass
                self.logger.error("Error in health check %s: %s", check['name'], e)
                check['last_check'] = datetime.datetime.now()  # local-ok
                check['status'] = 'error'
                check['message'] = str(e)
    ...
    def _save_health_status(self):
        ...
        try:
            if isoformat_z and utc_now:
                ts = isoformat_z(utc_now())
            else:
                raise ImportError("timeutils not available")
        except Exception:
