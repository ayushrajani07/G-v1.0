...
def _maybe_run_benchmark_cycle(metrics: BenchMetricsLike | None) -> None:
  ...
  if not EnvConfig.get_bool('G6_BENCH_CYCLE', False):
    return
  now = time.time()
  ...
  try:
    ...
    thr_env_pre = EnvConfig.get_str('G6_BENCH_P95_ALERT_THRESHOLD')
  ...
  if bench_mod is None:
    ...
    return
  indices_spec = EnvConfig.get_str('G6_BENCH_CYCLE_INDICES', 'NIFTY:1:1')
  cycles = EnvConfig.get_int('G6_BENCH_CYCLE_CYCLES', 8)
  warmup = EnvConfig.get_int('G6_BENCH_CYCLE_WARMUP', 2)
  ...
  thr_env = EnvConfig.get_str('G6_BENCH_P95_ALERT_THRESHOLD')
...
def run_pipeline(
  ...
  for index_symbol, cfg in index_params.items():
    ...
    try:
      ...
      for expiry_date in sorted(expiry_map.keys()):
        ...
        async_flag = EnvConfig.get_bool('G6_ENRICH_ASYNC', False) and _enrich_quotes_async is not None
  ...
  if EnvConfig.get_bool('G6_PIPELINE_PARITY_LOG', False) and legacy_baseline is not None:
  ...
  if snapshot_summary is not None and alert_summary is not None:
    try:
      ...
      if EnvConfig.get_bool('G6_ALERTS_FLAT_COMPAT', True):
  ...
  try:
    if EnvConfig.get_bool('G6_PIPELINE_MEMORY_GAUGE', True) and metrics is not None:
      from prometheus_client import Gauge as _G
  ...
  _include_diag = EnvConfig.get_bool('G6_PIPELINE_INCLUDE_DIAGNOSTICS', False)
  ret_obj = {
    'status': 'ok',
