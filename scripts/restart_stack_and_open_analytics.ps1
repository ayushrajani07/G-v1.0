param(
  ...
  [string]$V3Path = "$PSScriptRoot\..\data\analytics.json",
...
# Allow overriding Grafana data root via env var; default to C:\GrafanaData for compatibility
$GrafanaDataRoot = if ($env:G6_GRAFANA_ROOT -and $env:G6_GRAFANA_ROOT.Trim().Length -gt 0) { $env:G6_GRAFANA_ROOT } else { 'C:\GrafanaData' }
...
# Allow datasource plugins (like Infinity) to access localhost/lan targets
...
if ($UseV1) {
  ...
  # Prefer opening a lightweight smoke dashboard first for quick connectivity checks,
  # then fall back to main analytics variants (v3 -> v2 -> v1). v4 intentionally omitted.
  $preferredUids = @('g6-analytics-smoke','g6-analytics-infinity-v3','g6-analytics-infinity-v2','g6-analytics-infinity')
}
