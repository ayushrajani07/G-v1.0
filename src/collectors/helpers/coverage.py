...
_SUPPRESS_COVERAGE_WARN = EnvConfig.get_bool('G6_SUPPRESS_COVERAGE_WARNINGS', False)
    ...
    try:
        ...
        if coverage_ratio < 0.8:
            if not _SUPPRESS_COVERAGE_WARN:
                logger.warning(
                    "Instrument coverage low for %s %s %s: %s (realized=%s requested=%s)", index_symbol, expiry_rule, expiry_date, coverage_ratio, len(realized_strikes), len(strikes)
                )
        else:
            logger.debug(
                "Instrument coverage %s %s %s: %s (%s/%s)", index_symbol, expiry_rule, expiry_date, coverage_ratio, len(realized_strikes), len(strikes)
            )
    ...
    try:
        ...
        if total_options > 0:
            ...
            logger.debug(
                ...
                total_options,
                full_present,
                missing_counts['volume'],
                missing_counts['oi'],
                missing_counts['avg_price'],
                coverage_pct,
            )
            if coverage_pct < 60.0 and not _SUPPRESS_COVERAGE_WARN:
                logger.warning(
                    "Low option field coverage %s %s %s: %s%% (full=%s/%s)",
                    index_symbol,
                    expiry_rule,
                    expiry_date,
                    coverage_pct,
