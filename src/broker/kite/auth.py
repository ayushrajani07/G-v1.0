...
# Optional imports for late import elimination (Batch 36)
try:
    from .client_bootstrap import ensure_client as _ensure_client_boot
except ImportError:  # pragma: no cover
    _ensure_client_boot = None  # type: ignore
try:
    from .client_bootstrap import update_credentials as _update_creds_boot
except ImportError:  # pragma: no cover
    _update_creds_boot = None  # type: ignore
...
def ensure_client_auth(provider) -> None:  # pragma: no cover - thin delegate
    try:
        if _ensure_client_boot:
            _ensure_client_boot(provider)
    except Exception:
        pass
...
def update_credentials_auth(provider, api_key: str | None = None, access_token: str | None = None, rebuild: bool = True) -> None:
    try:
        if _update_creds_boot:
            _update_creds_boot(provider, api_key=api_key, access_token=access_token, rebuild=rebuild)
            return
    except Exception:
        pass
