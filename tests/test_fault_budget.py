...
try:
    from src.metrics.fault_budget import init_fault_budget as _init_fault_budget_import
except ImportError:
    _init_fault_budget_import = None  # type: ignore
...
def test_fault_budget_basic(monkeypatch):
    ...
    with isolated_metrics_registry() as reg:
        ...
        if tracker is None:
            ...
            if _init_fault_budget_import:
                _init_fault_budget_import(reg)
            tracker = getattr(reg, '_fault_budget_tracker', None)
