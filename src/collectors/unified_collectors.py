...
def _maybe_init_greeks(compute_greeks: bool, estimate_iv: bool, risk_free_rate: float, metrics: Any) -> tuple[Any, bool, bool]:
    ...
    if greeks_enabled_effective:
        try:
            ...
            if compute_greeks:
                logger.info("Greek computation enabled (r=%s) [force_env=%s]", risk_free_rate, _env_force_greeks)
            elif estimate_iv:
                logger.info("IV estimation enabled (r=%s) [force_env=%s]", risk_free_rate, _env_force_greeks)
