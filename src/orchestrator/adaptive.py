...
2. When breaches reach threshold, multiply active scale factor by reduction
    (not below min ratio of (min_depth/base_depth)).
3. Apply new scaled depths to each enabled index (mutate ctx.index_params in-place)
    and emit metric.
4. Healthy cycles increment healthy counter and reset breach counter. When healthy
    counter reaches restore threshold and scale < 1.0, restore toward 1.0 by dividing
    by reduction (or set to 1.0 if close). Reapply depths.
...
def _get_env_float(name: str, default: float) -> float:
    """Deprecated: Prefer EnvConfig.get_float() directly."""
...
def update_strike_scaling(ctx: Any, elapsed: float, interval: float) -> None:
    ...
    try:
        ...
        breach_threshold = EnvConfig.get_int('G6_ADAPTIVE_STRIKE_BREACH_THRESHOLD', 3)
        restore_threshold = EnvConfig.get_int('G6_ADAPTIVE_STRIKE_RESTORE_HEALTHY', 10)
        reduction = _get_env_float('G6_ADAPTIVE_STRIKE_REDUCTION', 0.8)
        min_depth = EnvConfig.get_int('G6_ADAPTIVE_STRIKE_MIN', 2)
