...
class CollectorSettings:
    ...
    def load(cls) -> CollectorSettings:
        return cls(
            min_volume=EnvConfig.get_int('G6_FILTER_MIN_VOLUME', 0),
            min_open_interest=EnvConfig.get_int('G6_FILTER_MIN_OI', 0),
            volume_percentile=EnvConfig.get_float('G6_FILTER_VOLUME_PERCENTILE', 0.0),
            salvage_enabled=not EnvConfig.get_bool('G6_DISABLE_SALVAGE', False),
            retry_on_empty=EnvConfig.get_bool('G6_RETRY_ON_EMPTY', False),
            trace_enabled=EnvConfig.get_bool('G6_TRACE_COLLECTOR', False),
            foreign_expiry_salvage=EnvConfig.get_bool('G6_FOREIGN_EXPIRY_SALVAGE', False),
            pipeline_v2_flag=EnvConfig.get_bool('G6_COLLECTOR_PIPELINE_V2', False),
            domain_models=EnvConfig.get_str('G6_DOMAIN_MODELS', '') or None,
        )
