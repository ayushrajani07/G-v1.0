...
def _mk_ctx():
    return RuntimeContext(config={}, index_params={'NIFTY': {'enable': False}}, providers=DummyProviders(), csv_sink=None, metrics=DummyMetrics())
...
def test_parallel_interval_inline_comment(monkeypatch):
    ...
    ctx = RuntimeContext(config={}, index_params={'A': {'enable': False}, 'B': {'enable': False}}, providers=DummyProviders(), csv_sink=None, metrics=DummyMetrics())
    from src.orchestrator import cycle as cycle_mod
    importlib.reload(cycle_mod)
    elapsed = cycle_mod.run_cycle(ctx)
    assert elapsed >= 0.0
