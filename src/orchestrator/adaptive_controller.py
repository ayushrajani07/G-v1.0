...
def evaluate_adaptive_controller(ctx, elapsed: float, interval: float) -> None:  # noqa: C901 complexity acceptable for initial version
    if not EnvConfig.get_bool('G6_ADAPTIVE_CONTROLLER', False):
        return
    ...
    # Prefer dynamic ctx flag populated by memory_pressure evaluator; fallback to env.
    memory_tier = 0
    try:
    ...
    if (
        action is None
        and state.detail_mode > max_detail_mode
        and state.healthy_streak >= recovery_cycles
        and not cardinality_active
        and memory_tier == 0
    ):
    ...
    if metrics:
        ...
        if hasattr(metrics, 'option_detail_mode'):
            # Helper to avoid try/except in hot loop
            from typing import Any as _Any
            def _safe_set_mode(index: str, value: int) -> None:
                try:
                    labeller = getattr(metrics.option_detail_mode, 'labels', None)
                    if not callable(labeller):
                        return
                    handle: _Any = labeller(index=index)
