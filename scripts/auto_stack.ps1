param(
  [bool]$StartPrometheus = $true,
  [bool]$StartGrafana = $true,
  [bool]$StartInflux = $true,
  [switch]$StartWebApi = $true,
  # Prometheus config: default to repo-local prometheus.yml; can be overridden by param
...
# Resolve repo root and sane defaults for parameters if not explicitly provided
$__repoRoot = Split-Path $PSScriptRoot -Parent
if (-not $PSBoundParameters.ContainsKey('PrometheusConfig') -or -not $PrometheusConfig -or -not (Test-Path -LiteralPath $PrometheusConfig)) {
  try { $PrometheusConfig = Join-Path $__repoRoot 'prometheus.yml' } catch {}
}
...
function Resolve-Python {
  ...
  try {
    if ($env:G6_PYTHON_EXE -and (Test-Path $env:G6_PYTHON_EXE)) {
      return [pscustomobject]@{Exe=$env:G6_PYTHON_EXE; Prefix=@()}
    }
  } catch {}
...
function Resolve-Grafana {
  # First, honor explicit override via environment variable
  try {
    if ($env:G6_GRAFANA_ROOT -and (Test-Path $env:G6_GRAFANA_ROOT)) {
      $exeFromEnv = Join-Path (Join-Path $env:G6_GRAFANA_ROOT 'bin') 'grafana-server.exe'
      if (Test-Path $exeFromEnv) { return [pscustomobject]@{ Home=$env:G6_GRAFANA_ROOT; Exe=$exeFromEnv } }
    }
  } catch {}
  $bases = @('C:\Program Files\GrafanaLabs\grafana','C:\Grafana','C:\Program Files','C:\Program Files (x86)')
  foreach ($b in $bases) {
    if (-not (Test-Path $b)) { continue }
    # First, check if this base itself is a Grafana home (has bin/grafana-server.exe)
    try {
      $exeDirect = Join-Path (Join-Path $b 'bin') 'grafana-server.exe'
      if (Test-Path $exeDirect) { return [pscustomobject]@{ Home=$b; Exe=$exeDirect } }
    } catch {}
    # Otherwise search child dirs
...
if (-not $promPort -and $StartPrometheus) {
  ...
  $promLogDir = Join-Path $GrafanaDataRoot 'log'
  Ensure-Dir -Path $promLogDir
  $pOut = Join-Path $promLogDir 'prometheus_stdout.log'
  $pErr = Join-Path $promLogDir 'prometheus_stderr.log'
  Start-Process -FilePath $promExe -ArgumentList $args -WorkingDirectory (Split-Path $PrometheusConfig -Parent) -RedirectStandardOutput $pOut -RedirectStandardError $pErr -WindowStyle Minimized
...
"@
          ...
          # Exclude ALL auto-generated dashboards - use only custom dashboards
          # Auto-generated dashboards are deprecated per user request (Phase 2 Optimization followup)
          $exclude = @('manifest.json','g6_spec_panels_dashboard.json','g6_generated_spec_dashboard.json','*.json')
          try {
            # Skip generated/ directory entirely - only provision custom dashboards
            if (Test-Path $repoDashGen) {
              # All generated dashboards are now excluded - comment out provisioning
...
function Start-PrometheusOnPort { param([string]$Exe,[int]$Port)
  ...
  $pOut = Join-Path $promLogDir 'prometheus_stdout.log'
  $pErr = Join-Path $promLogDir 'prometheus_stderr.log'
  Start-Process -FilePath $Exe -ArgumentList $args -WorkingDirectory (Split-Path $PrometheusConfig -Parent) -RedirectStandardOutput $pOut -RedirectStandardError $pErr -WindowStyle Minimized
}
