...
# Optional imports for event bus features
try:
...
class EventBus:
    ...
    def __init__(self, max_events: int = 2048) -> None:
        ...
        if AdaptiveController is not None:
            try:
    ...
    def _env_int(self, name: str, default: int) -> int:
        # Prefer centralized env adapter for consistent parsing; fallback to direct EnvConfig
        if _env_get_int is not None:
            try:
                return _env_get_int(name, default)
            except Exception:
                pass
    ...
    def publish(
        ...
        *,
        ...
        if serialize_event is not None:
            try:
                    ...
                    try:
                        ...
                        if isinstance(tr, dict) and 'serialize_ts' not in tr:
                            ...
                            try:
                                m = get_metrics()
                                if m:
                                    ctr = getattr(m, 'sse_trace_stages_total', None)
                                    inc = getattr(ctr, 'inc', None) if ctr else None
                                    if callable(inc):
                                        inc()
                            except Exception:
                                pass
        ...
        with self._lock:
            ...
            for ev in reversed(self._events):
                if ev.event_type == 'panel_full':
                    ...
                    break
            latest_id = self._seq
            gap_max = EnvConfig.get_int('G6_EVENTS_SNAPSHOT_GAP_MAX', 500)
    ...
    def _record_forced_full(self, reason: str) -> bool:
        ...
        try:
            ...
            return False
        cooldown = EnvConfig.get_float('G6_EVENTS_FORCE_FULL_RETRY_SECONDS', 30.0)
        with self._lock:
