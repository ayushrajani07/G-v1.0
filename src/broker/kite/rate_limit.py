...
def build_default_rate_limiter() -> RateLimiter:
    qps = EnvConfig.get_int('G6_KITE_QPS', 3)
    burst_env = EnvConfig.get_str('G6_KITE_RATE_MAX_BURST', '')
    burst = int(burst_env) if burst_env and burst_env.isdigit() else None
    thr = EnvConfig.get_int('G6_KITE_RATE_CONSECUTIVE_THRESHOLD', 5)
    cd = EnvConfig.get_int('G6_KITE_RATE_COOLDOWN_SECONDS', 20)
