...
# Optional imports
try:
    from src.collectors.pipeline import build_default_pipeline  # noqa: F401
except ImportError:
    build_default_pipeline = None  # type: ignore
try:
    from src.collectors import pipeline as pipeline_mod  # type: ignore
except ImportError:
    pipeline_mod = None  # type: ignore
try:
    from src.utils import strikes as strikes_mod  # type: ignore
except ImportError:
    strikes_mod = None  # type: ignore
try:
    from src.domain import snapshots_cache  # type: ignore
except ImportError:
    snapshots_cache = None  # type: ignore
try:
    from src.orchestrator import global_phase_timing as gpt_mod  # type: ignore
except ImportError:
    gpt_mod = None  # type: ignore
try:
    from src.adaptive.logic import evaluate_and_apply  # type: ignore
except ImportError:
    evaluate_and_apply = None  # type: ignore
try:
    from src.lifecycle.job import run_lifecycle_once  # type: ignore
except ImportError:
    run_lifecycle_once = None  # type: ignore
...
try:  # unified collectors (primary path)
    ...
    # Expose as Any-typed alias so we can safely replace with None when module
    # missing without mypy Callable assignment errors
    run_unified_collectors: Any = _run_uc
except (ImportError, AttributeError):  # pragma: no cover
    # Handle missing module or function
    run_unified_collectors: Any = None
# snapshot_collectors path removed after integration into unified collectors
# (placeholder kept for backward import safety)
...
_build_default_pipeline: Any = None
if _PIPELINE_FLAG:
    try:  # defer import cost until flag set
        if not build_default_pipeline:
            raise ImportError("build_default_pipeline not available")
        _build_default_pipeline = build_default_pipeline
    except (ImportError, AttributeError):  # pragma: no cover
        # Handle missing pipeline builder
        _build_default_pipeline = None
...
def _pipeline_run_expiry(pipe: Any, index: str, rule: str, strikes: Any, index_price: float, atm: float) -> Any:
    """Run a single pipeline expiry work item, returning the outcome or None.

    This helper encapsulates try/except so the caller's loop body remains clean
    (PERF203: avoid try/except inside loops). Behavior and logging remain
    ...
    try:
        ...
        try:
            if pipeline_mod:
                ExpiryWorkItem = getattr(pipeline_mod, 'ExpiryWorkItem', None)
            else:
                raise ImportError("pipeline module not available")
        except (ImportError, AttributeError):
            # Handle pipeline module or ExpiryWorkItem access failures
            ExpiryWorkItem = None
        ...
        logger.debug("pipeline expiry run failed index=%s rule=%s", index, rule, exc_info=True)
        return None
...
def run_cycle(ctx: RuntimeContext) -> float:
    ...
    _pit_raw = EnvConfig.get_str('G6_PARALLEL_INDEX_TIMEOUT_SEC', '')
    if _pit_raw:
        try:
    ...
    sla_fraction = EnvConfig.get_float('G6_CYCLE_SLA_FRACTION', 0.85)
    try:
        # Missing cycle detection: only advance reference timestamp when providers present.
        # Read factor and interval directly from os.environ to respect mid-test updates without EnvConfig cache.
        import os as _os
        _interval_raw = _os.environ.get('G6_CYCLE_INTERVAL', '')
        try:
            interval_env = float(_interval_raw) if _interval_raw.strip() else 60.0
        except (ValueError, TypeError, AttributeError):
            # Handle float conversion or string operation failures
            interval_env = 60.0
        raw_factor = _os.environ.get('G6_MISSING_CYCLE_FACTOR', '2.0')
        try:
        ...
        # Handle missing cycle detection failures
    ...
    if getattr(ctx, 'providers', None) is None:
        if not getattr(run_cycle, '_g6_warned_missing_providers', False):
            logger.warning(
                "run_cycle: providers missing; cycle producing NO_DATA "
                "(credentials or provider init required)"
            )
            try:
                run_cycle._g6_warned_missing_providers = True  # type: ignore[attr-defined]
            except (AttributeError, TypeError):
                # Handle attribute assignment failures
                pass
        return 0.0
    ...
    try:
        ...
        try:
            ...
            # Handle index_params access failures
            indices = []
        if parallel_enabled and len(indices) > 1:
            ...
            def submit_all(executor):
                ...
                for idx in indices:
                    ...
                    try:
                        ...
                        # Handle future attribute assignment failures
                        pass
                    fut_map[fut] = idx
                return fut_map
            ...
            if failures and retry_limit > 0 and remaining() > 0:
                for idx, _err in list(failures.items()):
                    if remaining() <= 0:
                        break
                    attempts = 0
                    ...
                    if callable(build_default_pipeline):
                        try:
                            ...
                            for _idx, _params in (ctx.index_params or {}).items():
                                ...
                                try:
                                    if strikes_mod:
                                        build_strikes = getattr(strikes_mod, 'build_strikes', None)
                                    else:
                                        raise ImportError("strikes module not available")
                                ...
                                for _rule in expiries:
                                    ...
                                    if (
                                        ...
                                        and outcome.pcr is not None
                                        and outcome.expiry_code
                                    ):
                                        overview_capture.setdefault(_idx, {})[
                                            outcome.expiry_code
                                        ] = outcome.pcr
                                        if (
                                            outcome.snapshot_timestamp
                                            and outcome.snapshot_timestamp.timestamp()
                                            < base_ts.get(_idx, float('inf'))
                                        ):
                                            base_ts[_idx] = (
                                                outcome.snapshot_timestamp.timestamp()
                                            )
                            ...
                            try:
                                for _idx, _pcrs in overview_capture.items():
                                    if _pcrs:
                                        ...
                                        snap_ts = (
                                            _dt.datetime.fromtimestamp(ts_val, _dt.UTC)
                                            if ts_val
    ...
    try:
        if EnvConfig.get_bool('G6_GLOBAL_PHASE_TIMING', False):
            try:
                if gpt_mod:
    ...
    try:
        if getattr(ctx, 'metrics', None):
            ...
            # last per-index success timestamps; fallback skip if absent.
            try:
    ...
    try:
        if is_truthy_env('G6_INTEGRITY_AUTO_RUN'):
            mod_val = EnvConfig.get_int('G6_INTEGRITY_AUTO_EVERY', 60)
            if mod_val <= 0:
                mod_val = 60
