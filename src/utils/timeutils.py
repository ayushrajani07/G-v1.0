...
# Timezones with robust fallback when tzdata is unavailable
UTC = _UTC
try:
    IST = ZoneInfo('Asia/Kolkata')
except (ZoneInfoNotFoundError, Exception):  # tzdata missing or other zoneinfo error
    try:
        import pytz  # type: ignore

        IST = pytz.timezone('Asia/Kolkata')  # type: ignore[assignment]
        try:
            logging.getLogger(__name__).warning('tzdata not found; using pytz fallback for Asia/Kolkata')
        except Exception:
            pass
    except Exception:
        # Fixed offset fallback (+05:30) if pytz is not available
        from datetime import timezone

        IST = timezone(timedelta(hours=5, minutes=30), name='IST')  # type: ignore[assignment]
        try:
            logging.getLogger(__name__).warning('tzdata not found; using fixed-offset IST (+05:30) fallback')
        except Exception:
            pass
...
def _weekend_mode() -> bool:
    ...
    """
    try:
        return EnvConfig.get_bool('G6_WEEKEND_MODE', False)
    except Exception:
        return False
