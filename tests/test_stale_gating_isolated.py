...
def test_stale_counter_isolated_between_registries(metrics_port):
    os.environ['G6_STALE_WRITE_MODE'] = 'mark'
    r1 = force_new_metrics_registry(enable_resource_sampler=False, port=metrics_port)
    ...
    # Use the same port since we're resetting (reset=True clears the old server)
    r2 = force_new_metrics_registry(enable_resource_sampler=False, port=metrics_port)
