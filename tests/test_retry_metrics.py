...
try:
    from src.collectors.errors import PhaseRecoverableError as _PhaseRecoverableError_import
except ImportError:
    _PhaseRecoverableError_import = None  # type: ignore
...
def _phase_builder(counter):
    def phase(ctx, st):
        counter['calls'] += 1
        if counter['calls'] < 3 and _PhaseRecoverableError_import:
            raise _PhaseRecoverableError_import('transient')
        return st
    return phase
