...
def iter_daily_rows(
    ...
    # Import enhanced cache for performance (Phase 8 optimization)
    try:
        from src.utils.enhanced_cache import get_enhanced_cache
    ...
    if cache:
        exists = cache.exists_cached(index_root)
    else:
    ...
    for expiry_tag_dir in index_root.iterdir():
        ...
        for offset_dir in expiry_tag_dir.iterdir():
            ...
            # Use cached exists check for daily file
            if cache:
                file_exists = cache.exists_cached(daily_file)
            else:
