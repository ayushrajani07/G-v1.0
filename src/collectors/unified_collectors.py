...
try:
    ...
    get_collector_settings = None  # type: ignore
try:
    from src.collectors.env_adapter import get_bool, get_str, get_int
except ImportError:
    get_bool = None  # type: ignore
    get_str = None  # type: ignore
    get_int = None  # type: ignore

# Optional imports for pipeline modules (Phase 3b Batch 23)
try:
    from src.collectors.modules import index_processor as index_processor_mod
except ImportError:
...
try:
    from src.collectors.modules.expiry_processor import process_expiry as process_expiry_impl
except ImportError:
    process_expiry_impl = None  # type: ignore
try:
    from src.collectors.modules.expiry_pipeline import pipeline_enabled, process_expiry_v2
except ImportError:
    pipeline_enabled = None  # type: ignore
    process_expiry_v2 = None  # type: ignore

# Optional imports for lifecycle helpers (Phase 3b Batch 24)
try:
    from src.collectors.modules.market_gate import evaluate_market_gate
except ImportError:
    evaluate_market_gate = None  # type: ignore
try:
    from src.collectors.modules.metrics_updater import finalize_cycle_metrics
except ImportError:
    finalize_cycle_metrics = None  # type: ignore
try:
    from src.collectors.modules.benchmark_bridge import write_benchmark_artifact
except ImportError:
    write_benchmark_artifact = None  # type: ignore
try:
    from src.collectors.modules.alerts_core import aggregate_alerts
except ImportError:
    aggregate_alerts = None  # type: ignore

# Optional imports for lightweight singles (Phase 3b Batch 25)
try:
    from src.broker.kite_provider import is_concise_logging
except ImportError:
    is_concise_logging = None  # type: ignore
try:
    from src.analytics.option_greeks import OptionGreeks
except ImportError:
    OptionGreeks = None  # type: ignore
try:
    from src.collectors.settings import CollectorSettings
except ImportError:
    CollectorSettings = None  # type: ignore
try:
    from src.orchestrator import global_phase_timing
except ImportError:
    global_phase_timing = None  # type: ignore
try:
    from src.logstream.formatter import format_cycle, format_cycle_readable, format_cycle_table, format_indices_table
except ImportError:
    format_cycle = None  # type: ignore
    format_cycle_readable = None  # type: ignore
    format_cycle_table = None  # type: ignore
    format_indices_table = None  # type: ignore

# Optional imports for complex singles (Phase 3b Batch 26)
try:
    from src.broker.kite.quote_batcher import get_batcher
except ImportError:
    get_batcher = None  # type: ignore
try:
...
try:
    ...
    compute_index_coverage = None  # type: ignore
    ...
    def _trace(msg: str, **ctx: Any) -> None:  # fallback minimal gating using CollectorSettings when available
        ...
        try:
            ...
            if settings_obj:
                ...
                if get_bool:
                    quiet_mode = get_bool('G6_QUIET_MODE', False)
                    quiet_allow_trace = get_bool('G6_QUIET_ALLOW_TRACE', False)
                    trace_enabled = get_bool('G6_TRACE_COLLECTOR', False)
                else:
                    quiet_mode = False
                    quiet_allow_trace = False
                    trace_enabled = False
    ...
    try:
        ...
        if settings_obj:
            ...
            if hb_val and hb_val > 0:
                ...
                if get_str:
                    interval_env = get_str('G6_LOOP_HEARTBEAT_INTERVAL','')
                else:
                    interval_env = ''
        else:
            if get_str:
        ...
        logger.info(
            ...
            (cycles if cycles is not None else 'NA'),
            (opts if opts is not None else 'NA'),
            (limiter_tokens if limiter_tokens is not None else 'NA'),
            (limiter_cooldown if limiter_cooldown is not None else 'NA'),
            (batch_saved if batch_saved is not None else 'NA'),
...
def _maybe_init_greeks(compute_greeks: bool, estimate_iv: bool, risk_free_rate: float, metrics: Any) -> tuple[Any, bool, bool]:
    ...
    if greeks_enabled_effective:
        try:
            if not OptionGreeks:
                raise ImportError("OptionGreeks not available")
            greeks_calculator = OptionGreeks(risk_free_rate=risk_free_rate)
            if compute_greeks:
                logger.info("Greek computation enabled (r=%s) [force-env=%s]", risk_free_rate, _env_force_greeks)
            elif estimate_iv:
                logger.info("IV estimation enabled (r=%s) [force-env=%s]", risk_free_rate, _env_force_greeks)
        except Exception as e:
            logger.error("Failed to initialize OptionGreeks: %s", e)
            if compute_greeks:
                compute_greeks = False
            if estimate_iv:
                estimate_iv = False
...
def _maybe_auto_disable_trace_flags() -> None:
    ...
    if _TRACE_AUTO_DISABLED or not EnvConfig.get_bool('G6_TRACE_AUTO_DISABLE', False):
        return
    noisy_flags = [
        'G6_TRACE_COLLECTOR',
    ...
    for flag in noisy_flags:
        try:
            val = EnvConfig.get_str(flag, '').lower()
...
def _default_rule_for_index(index_symbol: str) -> str:
    """Return the index-aware default expiry rule used in fallback paths.

    BANKNIFTY/FINNIFTY are monthly-only; default to this_month. Others default to this_week.
    """
    try:
...
def _resolve_expiry(index_symbol: str, expiry_rule: str, providers: Any, metrics: Any, concise_mode: bool) -> datetime.date:  # pragma: no cover
    ...
    Uses the cached selector in `collectors.modules.expiry_helpers.resolve_expiry`
    to avoid re-resolving expiry candidates every cycle.
    Any failure falls back to today's date (logged in non-concise mode).
    """
    import datetime as _dt
    import logging
    try:
        from src.collectors.modules.expiry_helpers import resolve_expiry as _impl
        return cast(datetime.date, _impl(index_symbol, expiry_rule, providers, metrics, concise_mode))
...
def _process_index(
    ...
    *,
    ...
    """
    try:
        if not index_processor_mod or not IndexProcessResult:
            raise ImportError("index_processor module not available")
        res: IndexProcessResult = index_processor_mod.process_index(  # type: ignore[valid-type]
            ctx,
            index_symbol,
            params,
...
def _process_expiry(
    *,
    ...
    try:
        if not process_expiry_impl:
            raise ImportError("expiry_processor module not available")
        # Optional pipeline v2 path
        try:
            if pipeline_enabled and process_expiry_v2:
...
def run_unified_collectors(
    ...
    *args: Any,
    ...
    #   run_unified_collectors(index_params, providers, csv_sink, metrics)
    #   run_unified_collectors(index_params, providers, csv_sink, influx_sink, metrics)
    #   run_unified_collectors(..., influx_sink=..., metrics=...)
    if args:
        # If 2+ extra positional args, treat them as (influx_sink, metrics, ...)
        if len(args) >= 2:
            ...
            # If only one extra positional, treat it as metrics (legacy 4th positional)
            if metrics is None:
                metrics = args[0]
    ...
    try:
        if isinstance(index_params, dict) and len(index_params) == 1:
            ...
            bypass_check = False
            if get_bool:
                bypass_check = get_bool('G6_FORCE_VALIDATION', False)
    ...
    try:
        if not evaluate_market_gate:
            raise ImportError("market_gate module not available")
    ...
    for index_symbol, params in index_params.items():
        try:
            ...
            logger.error("index_collect_failed index=%s err=%s", index_symbol, e)
            _res = {
                'summary_rows_entry': None,
                'human_block': None,
                'overall_legs': 0,
    ...
    if metrics:
        try:
            if not finalize_cycle_metrics:
                raise ImportError("metrics_updater module not available")
        ...
        try:
            ...
            if not cycle_ts_attr:
                try:
                    ...
                    logger.debug("Failed to convert cycle_ts to int: %s", e)
                    cycle_ts_attr = 0
    ...
    try:
        ...
        logger.debug("Failed to check stale status in indices: %s", e)
        stale_present = False
    if concise_mode and human_blocks:
        try:
            ...
            try:
                ...
                if get_collector_settings:
                    try:
                        ...
                        logger.debug("Failed to get provider_outage_threshold from settings: %s", e)
                        pass
                ...
                try:
                    ...
                    logger.debug("Failed to normalize outage_threshold: %s", e)
                    outage_threshold = 3
                ...
                try:
                    ...
                    logger.debug("Failed to check provider outage: %s", e)
                    ...
                    try:
                        ...
                        if get_collector_settings:
                            try:
                                ...
                                logger.debug("Failed to get provider_outage_log_every from settings: %s", e)
                                pass
                        if not throttle_n:
                            if get_int:
                        ...
                        logger.debug("Failed to set throttle_n: %s", e)
                        throttle_n = 5
                    try:
                ...
                logger.debug("Failed to process system status: %s", e, exc_info=True)
                pass
            ...
            if get_str and get_int:
                stale_mode = get_str('G6_STALE_WRITE_MODE', 'mark').lower()
                abort_cycles = get_int('G6_STALE_ABORT_CYCLES', 10)
            else:
                stale_mode = EnvConfig.get_str('G6_STALE_WRITE_MODE', 'mark').lower()
                abort_cycles = EnvConfig.get_int('G6_STALE_ABORT_CYCLES', 10)
            ...
            try:
                if metrics is not None:
                    ...
                    try:
                        ...
                        logger.debug("Failed to set _consec_stale_cycles: %s", e)
                else:
                ...
                if metrics is not None:
                    try:  # pragma: no cover - metrics wiring
                        ...
                        logger.debug('stale_system_metrics_failed: %s', e, exc_info=True)
                ...
                if stale_mode == 'abort' and stale_present and consec >= abort_cycles:
                    logger.critical("stale_abort_trigger system_status=%s consec_stale=%s threshold=%s", system_status, consec, abort_cycles)
                    try:
                        import sys as _sys
                        _sys.exit(32)
                    except SystemExit:
                        ...
                        logger.error("Failed to execute sys.exit: %s", e)
            except (AttributeError, TypeError, NameError) as e:
                logger.debug("Failed stale abort evaluation: %s", e, exc_info=True)
        except (AttributeError, TypeError, KeyError, ValueError, OSError) as e:
            logger.debug("Failed to emit human summary footer: %s", e, exc_info=True)
        ...
        try:
            ...
            if metrics is not None:
                ...
                try:
                    ...
                    logger.debug("Failed to set _consec_stale_cycles in fallback: %s", e)
            else:
                consec = 1 if stale_present else 0
            if stale_mode == 'abort' and stale_present and consec >= abort_cycles:
                logger.critical("stale_abort_trigger system_status=UNKNOWN consec_stale=%s threshold=%s", consec, abort_cycles)
                try:
                    import sys as _sys
                    _sys.exit(32)
                except SystemExit:
                    ...
                    logger.error("Failed to execute sys.exit in fallback: %s", e)
        except (AttributeError, TypeError, NameError) as e:
            logger.debug("stale_abort_evaluation_failed_fallback: %s", e, exc_info=True)
    ...
    try:
        if not format_cycle or not format_cycle_readable or not format_cycle_table:
            raise ImportError("formatter functions not available")
        legacy_disable = is_truthy_env('G6_DISABLE_PRETTY_CYCLE')
        if get_str:
            mode = 'raw' if legacy_disable else get_str('G6_CYCLE_OUTPUT', 'pretty').lower()
        else:
            mode = 'raw' if legacy_disable else EnvConfig.get_str('G6_CYCLE_OUTPUT', 'pretty').lower()
        if mode not in ('pretty','raw','both'):
            mode = 'pretty'
        if get_str:
            cycle_style = get_str('G6_CYCLE_STYLE','legacy').lower()  # legacy | readable
        else:
        ...
        if mode in ('raw','both'):
            try:
                ...
                logger.debug("Failed to format raw cycle line: %s", e, exc_info=True)
            ...
            try:
                header_line, value_line = format_cycle_table(
                    ...
                    ist_ts=(
                        ...
                        if (EnvConfig.get_bool('G6_CYCLE_SHOW_IST_TS', EnvConfig.get_bool('G6_HUMAN_MODE', False)))
                        else None
                    ),
                )
                # Expose latest header for other human emitters (e.g., PHASE_TIMING block footer).
                try:
                    if metrics is not None:
                        setattr(metrics, '_last_cycle_table_header_line', header_line)
                except Exception:
                    pass
                # Human mode defaults to a cleaner stream: print header once, then only the value row.
                try:
                    human_mode = EnvConfig.get_bool('G6_HUMAN_MODE', False)
                    header_once = EnvConfig.get_bool('G6_CYCLE_TABLE_HEADER_ONCE', human_mode)
                except Exception:
                    human_mode = False
                    header_once = False

                if header_once:
                    # Attach state on metrics when available; otherwise fallback to a module-level sentinel.
                    already = False
                    try:
                        if metrics is not None and hasattr(metrics, '_cycle_table_header_emitted'):
                            already = bool(getattr(metrics, '_cycle_table_header_emitted', False))
                        elif metrics is not None:
                            already = False
                    except Exception:
                        already = False
                    # Periodic header refresh to keep mid-scroll readable.
                    try:
                        header_every = EnvConfig.get_int('G6_CYCLE_TABLE_HEADER_EVERY', 0)
                    except Exception:
                        header_every = 0
                    try:
                        n_since = int(getattr(metrics, '_cycle_table_rows_since_header', 0)) if metrics is not None else 0
                    except Exception:
                        n_since = 0

                    emit_header_now = (not already)
                    if header_every and header_every > 0 and n_since >= header_every:
                        emit_header_now = True

                    if emit_header_now:
                        pretty_line = f"{header_line}\n{value_line}"
                        try:
                            if metrics is not None:
                                setattr(metrics, '_cycle_table_header_emitted', True)
                                setattr(metrics, '_cycle_table_rows_since_header', 0)
                        except Exception:
                            pass
                    else:
                        pretty_line = value_line
                        try:
                            if metrics is not None:
                                setattr(metrics, '_cycle_table_rows_since_header', n_since + 1)
                        except Exception:
                            pass
                else:
                    pretty_line = f"{header_line}\n{value_line}"
            except (TypeError, ValueError, AttributeError, KeyError) as e:
                logger.debug("Failed to format pretty cycle table: %s", e, exc_info=True)
        ...
        try:
            ...
            logger.debug("Failed to emit raw cycle line: %s", e, exc_info=True)
        _trace("cycle_raw_emitted", have_raw=bool(raw_line))
        try:
            if pretty_line:
                logger.info(pretty_line)
        except (OSError, ValueError) as e:
            logger.debug("Failed to emit pretty cycle summary: %s", e, exc_info=True)
        _trace("cycle_pretty_emitted", have_pretty=bool(pretty_line))

        # Optional blank spacer between blocks for terminal readability.
        try:
            if EnvConfig.get_bool('G6_HUMAN_MODE', False) and EnvConfig.get_bool('G6_HUMAN_SPACERS', True):
        ...
        # Optional: emit a compact per-index table for better operator readability.
        try:
            human_mode = EnvConfig.get_bool('G6_HUMAN_MODE', False)
            show_ix_table = EnvConfig.get_bool('G6_HUMAN_SHOW_INDEX_TABLE', human_mode)
        except Exception:
            show_ix_table = False
        if show_ix_table and format_indices_table and isinstance(indices_struct, list):
            try:
                max_rows = EnvConfig.get_int('G6_HUMAN_INDEX_TABLE_MAX_ROWS', 8)
            except Exception:
                max_rows = 8
            try:
                block = format_indices_table(cast(list[dict[str, Any]], indices_struct), max_rows=max_rows)
                logger.info(block)
                try:
                    if EnvConfig.get_bool('G6_HUMAN_MODE', False) and EnvConfig.get_bool('G6_HUMAN_SPACERS', True):
                        n = EnvConfig.get_int('G6_HUMAN_SPACER_LINES', 1)
                ...
                logger.debug('human_index_table_emit_failed: %s', e, exc_info=True)

        # End-of-cycle IST timestamp (operator-friendly; useful for screenshots and audits).
        try:
            human_mode = EnvConfig.get_bool('G6_HUMAN_MODE', False)
            if EnvConfig.get_bool('G6_CYCLE_END_IST', human_mode):
                ist_now = datetime.datetime.now(datetime.UTC).astimezone(_IST_TZ)
                logger.info("IST_END %s", ist_now.strftime('%Y-%m-%d %H:%M:%S'))
        except Exception:
            pass
    except (AttributeError, TypeError, NameError) as e:
        logger.debug("Failed to emit cycle line(s): %s", e, exc_info=True)
    # Optional heartbeat
    try:
        _maybe_emit_heartbeat(metrics)
    except (AttributeError, TypeError, NameError) as e:
        logger.debug('heartbeat_invoke_failed: %s', e, exc_info=True)
    ...
    try:
        ...
        logger.debug('trace_auto_disable_post_failed: %s', e, exc_info=True)
    ...
    try:
        ...
        logger.debug('indices_struct_safety_append_failed: %s', e, exc_info=True)
    ...
    try:
        ...
        try:
            if not write_benchmark_artifact:
                raise ImportError("benchmark_bridge module not available")
            ...
            logger.debug("benchmark_bridge_failed: %s", e, exc_info=True)
        ...
        try:
            ...
            logger.debug('legacy_alert_aggregation_failed: %s', e, exc_info=True)
            _alert_summary = None
        ...
        if snapshot_summary is not None and _alert_summary is not None:
            try:
                ...
                logger.debug('legacy_alert_snapshot_merge_failed: %s', e, exc_info=True)
    ...
    # provider_outage already defined above; leave as-is
        ret_obj = {
            ...
            'provider_outage_threshold': EnvConfig.get_int('G6_PROVIDER_OUTAGE_THRESHOLD', 3),
        }
        ...
        try:
            ...
            logger.debug('legacy_operational_metrics_failed: %s', e, exc_info=True)
        ...
        try:
            if not compute_index_coverage:
                raise ImportError("coverage_core module not available")
        ...
        logger.debug('structured_return_build_failed: %s', e, exc_info=True)
        try:
            return {
                'status': 'ok',
                'indices_processed': len(index_params or {}),
            ...
            logger.debug('fallback_return_build_failed: %s', e, exc_info=True)
            return {
                'status': 'ok',
                'indices_processed': len(index_params or {}),
                'have_raw': True,
