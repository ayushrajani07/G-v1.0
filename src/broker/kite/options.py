...
from src.collectors.helpers.struct_events import emit_instrument_prefilter_summary
from src.utils.strike_index import build_strike_index
from src.utils.root_cache import cached_detect_root
from src.broker.kite.settings import load_settings
from src.broker.kite.state import ProviderState
try:
    from src.utils.index_registry import get_index_meta  # weekly_dow policy per index
...
def prefilter_option_universe(index_symbol: str, instruments: Sequence[Instrument], raw_total_opts: int) -> tuple[list[Instrument], bool, dict[str, int]]:
    ...
    try:
        _pf_detect_root = detect_root
        _pf_parse_root = parse_root_before_digits
    except Exception:  # pragma: no cover
...
def match_options(provider: ProviderLike | Any, index_symbol: str, expiry_target: _dt.date, strikes: Sequence[float], instruments: Sequence[Instrument], pre_index: Mapping[tuple[_dt.date, float, str], Sequence[Instrument]], strike_membership: StrikeMembership) -> MatchResult:
    # Note: cached_detect_root is now available at module level
...
def apply_expiry_fallbacks(provider: ProviderLike | Any, index_symbol: str, expiry_target: _dt.date, strikes: Sequence[float], instruments: Sequence[Instrument], strike_membership: StrikeMembership, existing_matches: list[Instrument]) -> list[Instrument]:
    ...
    # cached_detect_root already available at module level
    ...
    # Determine if this request is a weekly request (aligns with index weekly DOW)
    weekly_dow = 3
    try:
        if get_index_meta is not None:
            weekly_dow = int(get_index_meta(index_symbol).weekly_dow)
    except Exception:
        weekly_dow = 3
    is_weekly_request = isinstance(expiry_target, _dt.date) and expiry_target.weekday() == weekly_dow
    # Forward fallback
    if settings and getattr(settings, 'enable_nearest_expiry_fallback', True):
        try:
            forward_candidates = sorted(
                e for e, m in by_exp_meta.items()
                if e >= expiry_target
                and (m.get('strike_set') or set()) & strike_key_set
                and (not is_weekly_request or e.weekday() == weekly_dow)
            )
...
def _log_summary(index_symbol: str, expiry_date: Any, strikes: Sequence[float], matching_instruments: Sequence[Instrument]) -> None:
    ...
    if is_concise_logging():
        ...
        logger.info("+ Options for %s (Expiry: %s) %s", index_symbol, expiry_date, "-" * 30)
        logger.info("| Found %s matching instruments", len(matching_instruments))
        if strikes_summary:
            ...
            for strike in sorted(strikes_summary.keys()):
                ...
                logger.info("| %s %s  %s", strike, ce_count, pe_count)
        logger.info("+" + "-" * 50)
...
def option_instruments(provider: ProviderLike | Any, index_symbol: str, expiry_date: Any, strikes: Iterable[float]) -> list[Instrument]:
    ...
    try:
        ...
        if cache_hit_complete and cached_all:
            ...
            if is_concise_logging():
                logger.debug("Instrument cache HIT idx=%s expiry=%s strikes=%s size=%s", index_symbol, expiry_iso, len(strikes_list), len(cached_all))
            return cached_all
        else:
            state.option_cache_misses += 1
            if is_concise_logging() and (state.option_cache_misses % 50 == 1):
                logger.debug("Instrument cache MISS idx=%s expiry=%s strikes=%s (miss #%s)", index_symbol, expiry_iso, len(strikes_list), state.option_cache_misses)
        ...
        # Weekly context for downstream fallbacks in this function
        try:
            _weekly_dow_local = int(get_index_meta(index_symbol).weekly_dow) if get_index_meta else 3
        except Exception:
            _weekly_dow_local = 3
        _is_weekly_request_local = isinstance(expiry_target, _dt.date) and expiry_target.weekday() == _weekly_dow_local
            ...
            if is_truthy_env('G6_RELAX_EMPTY_MATCH') or 'G6_RELAX_EMPTY_MATCH' not in os.environ:
                try:
                    ...
                    if not relaxed:
                        ...
                        if by_exp:
                            chosen: _dt.date | None = None
                            if _is_weekly_request_local:
                                # Preserve weekly weekday strictly: only consider expiries matching weekly_dow
                                forward = sorted(
                                    e for e in by_exp.keys()
                                    if e >= expiry_target and getattr(e, 'weekday', lambda: -1)() == _weekly_dow_local
                                )
                                if forward:
                                    chosen = forward[0]
                                else:
                                    # Optionally try a backward same-weekday candidate; still preserve weekday
                                    backward = sorted(
                                        (e for e in by_exp.keys() if e < expiry_target and getattr(e, 'weekday', lambda: -1)() == _weekly_dow_local),
                                        reverse=True
                                    )
                                    chosen = backward[0] if backward else None
                            else:
                                # Non-weekly requests can relax to nearest forward expiry (any weekday)
        ...
        logger.error("Failed to get option instruments: %s", e, exc_info=True)
        try:
            handle_error(
                e,
                ...
                context={
                    "component": "kite.options.option_instruments",
