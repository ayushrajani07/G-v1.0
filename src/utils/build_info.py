...
try:
    ...
    register_build_info = None  # type: ignore
...
def compute_git_commit(repo_root: str | None = None) -> str:
    ...
    env_commit = EnvConfig.get_str('G6_GIT_COMMIT', '')
    if env_commit:
...
def compute_version() -> str:
    env_version = EnvConfig.get_str('G6_VERSION', '')
    if env_version:
        return env_version
...
def auto_register_build_info(metrics, config: Any, repo_root: str | None = None):
    ...
    try:
        if metrics is None or register_build_info is None:
            return
