...
def _try_parse_date(fmt: str, value: object) -> datetime.date | None:
    try:
...
def _parse_date(d) -> datetime.date | None:
    ...
    for fmt in ('%Y-%m-%d', '%d-%m-%Y', '%Y%m%d'):
        parsed = _try_parse_date(fmt, d)
        if parsed is not None:
            return parsed
    return None


def validate_option_batch(
    index: str,
    expiry_rule: str,
    expiry_date,
    ...
    # If any hard issues (foreign_expiry, non_numeric_strike, etc.) occurred in
    # volume, but ok wasn't flipped by coverage, keep ok True unless all dropped
