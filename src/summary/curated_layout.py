...
def env_true(name: str) -> bool:
    """Truthiness helper that reads directly from the environment.

    Using os.environ ensures mid-test env mutations (monkeypatch) are observed
    without relying on any caching layer.
    """
...
def build_cycle(st: SummaryState) -> Block:
    line1 = (
        f"CYC t={_fmt_ms(st.cycle_last_duration_ms)} "
        f"p95={_fmt_ms(st.cycle_p95_ms)} "
        f"SLA={_fmt_ms(st.sla_ms)} "
        f"streak={st.sla_breach_streak} miss={st.misses}"
    )
    next_hint = (
        f"{st.next_run_in:.1f}s" if isinstance(st.next_run_in, (int, float)) else "?"
    )
    interval_hint = f"{st.cycle_interval:.0f}s" if st.cycle_interval else "?"
    line2 = f"ON={_fmt_pct(st.on_time_pct)} next={next_hint} interval={interval_hint}"
    ...
    def _shrink(b: Block):
        if len(b.lines) > 1:
            b.lines = [
                (
                    "CYC "
                    f"{_fmt_ms(st.cycle_last_duration_ms)}/"
                    f"{_fmt_ms(st.cycle_p95_ms)} SLA "
                    f"streak={st.sla_breach_streak} miss={st.misses} "
                    f"on={_fmt_pct(st.on_time_pct)}"
                )
            ]
    blk.shrink = _shrink
    return blk
...
def build_indices(st: SummaryState) -> Block:
    ...
    def _shrink(b: Block):
        if st.indices:
            first = st.indices[0]
            name = first.get('name', '?')
            change = _fmt_delta(first.get('change_pct'))
            rows = _fmt_num(first.get('rows'))
            ok = _fmt_pct(first.get('ok_pct'))
            b.lines = [f"IDX {name} {change} r={rows} ok={ok}"]
    blk.shrink = _shrink
    return blk
...
def build_provider(st: SummaryState) -> Block:
    ...
    line = (
        "PROV "
        f"p95={_fmt_ms(kite.get('lat_p95'))} "
        f"err={_fmt_pct(kite.get('err_pct'))} "
        f"cb={kite.get('cb') or '-'} | INF "
        f"p95={_fmt_ms(inf.get('p95'))} "
        f"q={_fmt_num(inf.get('q'))} "
        f"drop={_fmt_num(inf.get('drop'))}"
    )
    ...
    def _shrink(b: Block):
        b.lines = [
            (
                "NET "
                f"p95={_fmt_ms(kite.get('lat_p95'))} "
                f"err={_fmt_pct(kite.get('err_pct'))} | inf "
                f"p95={_fmt_ms(inf.get('p95'))}"
            )
        ]
    blk.shrink = _shrink
...
def build_dq_card(st: SummaryState) -> Block:
    card_ratio = (
        _fmt_num(st.card_active) + '/' + _fmt_num(st.card_budget)
        if st.card_active is not None and st.card_budget is not None
        else '?'
    )
    line = (
        f"DQ {_fmt_pct(st.dq_score)} "
        f"warn={_fmt_pct(st.dq_warn)} "
        f"err={_fmt_pct(st.dq_err)} | CARD {card_ratio} "
        f"dis={_fmt_num(st.card_disabled)} "
        f"atm={_fmt_num(st.atm_window)} "
        f"rate={_fmt_num(st.emit_rate)}/s"
    )
    blk = Block(key="dq_card", importance=75, lines=[line], shrinkable=True)
    def _shrink(b: Block):
        b.lines = [f"DQ {_fmt_pct(st.dq_score)} | CARD {card_ratio}"]
    blk.shrink = _shrink
    return blk

def build_memory(st: SummaryState) -> Block:
    rss_txt = (_fmt_num(st.rss_mb) + 'MB') if st.rss_mb is not None else '?'
    rb = st.rollback_in if st.rollback_in is not None else '-'
    line = (
        f"MEM {rss_txt} "
        f"tier={st.mem_tier or '-'} "
        f"head={_fmt_pct(st.headroom_pct)} "
        f"cpu={_fmt_pct(st.cpu_pct)} "
        f"rollback_in={rb}"
    )
    blk = Block(key="memory", importance=70, lines=[line], shrinkable=True)
    def _shrink(b: Block):
        b.lines = [f"MEM {rss_txt} {st.mem_tier or '-'} head={_fmt_pct(st.headroom_pct)}"]
    blk.shrink = _shrink
    return blk
...
def build_alerts(st: SummaryState) -> Block:
    ...
    line1 = (
        "ALERTS "
        f"i/w/c={_fmt_num(cts.get('info',0))}/"
        f"{_fmt_num(cts.get('warn',0))}/"
        f"{_fmt_num(cts.get('critical',0))} "
        f"types={_fmt_num(st.alerts_types)} "
        f"resolved={_fmt_num(st.alerts_resolved)}"
    )
    reasons = ",".join(st.adaptive_reasons[:2]) if st.adaptive_reasons else "none"
    line2 = (
        f"ADAPT mode={st.adaptive_mode or '-'} "
        f"demote_in={st.demote_in} promote_in={st.promote_in} "
        f"reasons:{reasons}"
    )
...
def build_analytics(st: SummaryState) -> Block:
    ...
    line = (
        "AN "
        f"vol_cov={_fmt_pct(vs.get('cov'))} "
        f"interp={_fmt_pct(vs.get('interp'))} "
        f"atm_iv={_fmt_pct(vs.get('atm_iv'))} | risk "
        f"Δ={_fmt_num(rk.get('delta'))} "
        f"vega={_fmt_num(rk.get('vega'))} "
        f"drift={_fmt_pct(rk.get('drift'))}"
    )
...
def build_heartbeat(st: SummaryState) -> Block:
    ...
    line = (
        "HB "
        f"evt={_fmt_num(hb.get('last_event_s'))}s "
        f"metrics={_fmt_num(hb.get('metrics_age_s'))} "
        f"p95:{hb.get('p95_spark') or '-'}"
    )
