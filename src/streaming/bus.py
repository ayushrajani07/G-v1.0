...
Goals:
- Provide a minimal interface decoupling producers/consumers from concrete
    transport (in-memory, future Redis/NATS, etc.)
- Surface consistent metrics naming (g6_stream_*) aligned with existing
    in-memory bus metrics but transport-neutral.
- Support simple fan-out subscription with optional type filtering.
- Keep semantics at-most-once (same as current in-memory) initially;
    allow future extension for durability.

Design Notes:
- We intentionally do NOT bake persistence/acks yet; interface includes a
    placeholder ack for forward compatibility.
- Event schema hashing: we emit a gauge labelled by event type and
    schema_hash (stable hash of sorted payload keys) so governance can detect
    drift.
- External backend selection is deferred to a factory using env var
    G6_STREAM_BACKEND (default: INMEM).
"""
import hashlib
import os
import threading
...
class AbstractStreamBus(Protocol):
    ...
    def publish(
        ...
        *,
        key: str | None = None,
        meta: dict[str, Any] | None = None,
    ) -> StreamPublishResult: ...  # pragma: no cover
    def subscribe(
        self,
        *,
        filter_fn: Callable[[str], bool] | None = None,
        from_id: int | None = None,
    ) -> StreamSubscription: ...  # pragma: no cover
...
class _StubExternalStreamBus(AbstractStreamBus):
    ...
    def publish(
        ...
        *,
        key: str | None = None,
        meta: dict[str, Any] | None = None,
    ) -> StreamPublishResult:  # pragma: no cover - simple stub
    ...
    def subscribe(
        ...
        *,
        filter_fn: Callable[[str], bool] | None = None,
        from_id: int | None = None,
    ) -> StreamSubscription:  # pragma: no cover - stub
...
def get_stream_bus(name: str = 'core') -> AbstractStreamBus:
    backend = EnvConfig.get_str('G6_STREAM_BACKEND', 'INMEM').upper()
    key = f"{backend}:{name}"
    with _backend_lock:
        if key in _backends:
