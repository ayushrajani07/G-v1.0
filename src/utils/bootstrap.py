...
# Optional imports
try:
    from src.error_handling import handle_data_error, handle_api_error  # type: ignore
except ImportError:
...
def load_raw_config(path: str) -> dict:
    ...
    try:
        ...
        if isinstance(data, dict):
            return data
        # If malformed or not a dict, fall back to minimal default
        return {"storage": {"csv_dir": "data/g6_data"}}
...
def bootstrap(
    ...
    if is_truthy_env:
        ...
        config_loader_flag = is_truthy_env('G6_CONFIG_LOADER')
    else:
        enhanced_cfg = False
        config_loader_flag = False
    if enhanced_cfg or config_loader_flag:
        try:
    ...
    if enable_metrics and config.get("metrics", {}).get("enabled", True):  # type: ignore[index]
        ...
        try:
            if is_truthy_env:
                ...
                enable_circuit_metrics = bool(config.get('resilience', {}).get('circuit_metrics', {}).get('enabled', False))  # type: ignore[index]
        ...
        try:
            if is_truthy_env:
                ...
                enable_health_api = bool(config.get('health', {}).get('api', {}).get('enabled', False))  # type: ignore[index]
                enable_health_prom = bool(config.get('health', {}).get('prometheus', {}).get('enabled', False))  # type: ignore[index]
        ...
        try:
            if is_truthy_env:
                ...
                enable_alerts = bool(config.get('health', {}).get('alerts', {}).get('enabled', False))  # type: ignore[index]
            if enable_alerts:
