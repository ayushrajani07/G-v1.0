...
def _enabled() -> bool:
    return EnvConfig.get_bool('G6_LIFECYCLE_JOB', False)
...
def _retention_prune(base: pathlib.Path, metrics, now: float) -> int:
    ...
    days = EnvConfig.get_float('G6_LIFECYCLE_RETENTION_DAYS', 0.0)
    if days <= 0:
        return 0
    limit = EnvConfig.get_int('G6_LIFECYCLE_RETENTION_DELETE_LIMIT', 100)
...
def run_lifecycle_once(base_dir: str = 'data/g6_data') -> None:
    ...
    try:
        ...
        pass
    exts_env = EnvConfig.get_str('G6_LIFECYCLE_COMPRESSION_EXT', '.csv')
    exts = {e if e.startswith('.') else f'.{e}' for e in exts_env.split(',') if e.strip()}
    age = EnvConfig.get_float('G6_LIFECYCLE_COMPRESSION_AGE_SEC', 86400.0)
    limit = EnvConfig.get_int('G6_LIFECYCLE_MAX_PER_CYCLE', 25)
    ...
    q_dir = pathlib.Path(EnvConfig.get_str('G6_LIFECYCLE_QUAR_DIR','data/quarantine'))
    start = time.time()
    if q_dir.exists():
        # walk limited depth
