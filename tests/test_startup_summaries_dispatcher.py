...
try:
    from src.utils.summary_json import emit_summary_json as _emit_summary_json_import  # type: ignore
except ImportError:
...
def test_emit_and_register_summary_composite_hash():
    ...
    def _emitter():
        if _emit_summary_json_import:
            _emit_summary_json_import('helper.test', [('alpha', 1), ('beta', 2)], logger_override=logging.getLogger('test.helper'))
        return True
