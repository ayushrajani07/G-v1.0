...
try:
    from src.broker.kite_provider import DummyKiteProvider as _DummyKiteProviderImport, KiteProvider as _KiteProviderImport
except ImportError:
    _DummyKiteProviderImport = None  # type: ignore
    _KiteProviderImport = None  # type: ignore
try:
    from src.utils.bootstrap import bootstrap as _bootstrap_import
except ImportError:
    _bootstrap_import = None  # type: ignore
...
def cmd_provider_check(args: argparse.Namespace) -> int:
    ...
    try:
        if not _DummyKiteProviderImport or not _KiteProviderImport or not _bootstrap_import:
            print(json.dumps({"bootstrap_import_error": "Required modules not available"}))
            return 1
        DummyKiteProvider = _DummyKiteProviderImport
        KiteProvider = _KiteProviderImport
        bootstrap = _bootstrap_import
    except Exception as e:
    ...
    try:
        ...
        try:
            if not _DummyKiteProviderImport:
                print(json.dumps({"dummy_provider_error": "DummyKiteProvider not available"}))
                return 1
            dp = _DummyKiteProviderImport()
            instruments=[('NSE','NIFTY 50')]
            try:
...
def cmd_one_cycle(args: argparse.Namespace) -> int:
    ...
    start = time.time()
    try:
        fn = build_cycle_fn()  # Factory returns cycle function
        fn()  # one cycle  # type: ignore[call-arg]
