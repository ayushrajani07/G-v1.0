...
try:
    from src.orchestrator.adaptive import update_strike_scaling as _update_strike_scaling_import
except ImportError:
    _update_strike_scaling_import = None  # type: ignore
...
def _run_cycles(ctx, elapsed_values, interval):
    ...
    for e in elapsed_values:
        ...
        if _update_strike_scaling_import:
            _update_strike_scaling_import(ctx, e, interval)
        ctx.cycle_count += 1
