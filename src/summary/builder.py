...
def build_summary_snapshot(
    *,
    ...
    raw_alerts : iterable of dict
        Each dict should contain at least 'code' and 'message'; optional
        'severity', 'index', 'meta'.
    raw_indices : mapping index -> dict of fields
        (atm, iv, status, success_rate, options_last_cycle, last_update_epoch).
    ...
    for name, spec in (raw_indices or {}).items():
        ...
        idx_health = IndexHealth(
            ...
            success_rate_percent=(
                float(spec['success_rate_percent'])
                if isinstance(spec.get('success_rate_percent'), (int, float))
                else None
            ),
            options_last_cycle=(
                int(spec['options_last_cycle'])
                if isinstance(spec.get('options_last_cycle'), (int, float))
                else None
            ),
