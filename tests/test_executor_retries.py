...
try:
    from src.metrics.metrics import MetricsRegistry as _MetricsRegistryImport
except ImportError:
    _MetricsRegistryImport = None  # type: ignore
...
def test_executor_retry_success(monkeypatch):
    ...
    try:
        if not _MetricsRegistryImport:
            return
        reg = _MetricsRegistryImport()
