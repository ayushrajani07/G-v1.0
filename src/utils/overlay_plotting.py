...
def env_int(name: str, default_val: int) -> int:
    try:
        return EnvConfig.get_int(name, default_val)
    except Exception:
        return default_val


# (typing imports consolidated at top)
...
def proc_mem_mb() -> float:
    if psutil is None:
        return -1.0
    try:
        rss_raw = cast(int, psutil.Process(os.getpid()).memory_info().rss)
        rss_bytes: float = float(rss_raw)
...
def monitor_memory(label: str, threshold_mb: int | None) -> None:
    ...
    if cur > 0 and cur > threshold_mb:
        logger.warning("High memory usage at %s: %.1fMB (threshold %sMB)", label, cur, threshold_mb)
        gc.collect()
...
def load_config_json(path: str | None) -> dict[str, Any]:
    if not path:
        return {}
    try:
        data = read_json_cached(Path(path))
        if isinstance(data, dict):
            return cast(dict[str, Any], data)
        return {}
    except Exception as e:
        logger.warning("Could not load JSON config %s: %s", path, e)
        return {}
...
try:
    ...
    # ensure_utc_helpers returns (utc_now: Callable[[], Any], isoformat_z: Callable[[Any], str])
    _utc_now_func: Callable[[], Any]
    _isoformat_z_func: Callable[[Any], str]
    _utc_now_func, _isoformat_z_func = ensure_utc_helpers()

    def utc_now() -> Any:
        return _utc_now_func()

    def isoformat_z(ts: Any) -> str:
        return _isoformat_z_func(ts)
    ...
    def isoformat_z(ts: Any) -> str:
        try:
            return cast(str, ts.isoformat().replace('+00:00', 'Z'))
        except Exception:
            return str(ts)
...
def export_figure_image(fig: Figure, path: Path, label: str) -> None:
    ...
    try:
        ...
        if not _STATIC_EXPORT_WARNED:
            logger.warning("static export disabled after failure on %s: %s", label, e)
            logger.info("Install 'kaleido' (pip install kaleido) to enable PNG export.")
