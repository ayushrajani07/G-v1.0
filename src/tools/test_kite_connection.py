...
def get_expiry(kite_provider: KiteProvider) -> list[str]:
    ...
    try:
        ...
        logger.warning("Error fetching expiries: %s", e)
        try:
            if handle_provider_error is not None:
                handle_provider_error(
                    e,
                    component="tools.test_kite_connection",
                    index_name="NIFTY",
                    context={"op": "get_expiry"},
                )
        except Exception:
            pass
    return []
...
def main() -> int:
    ...
    try:
        ...
        logger.error("Failed to init KiteProvider: %s", e)
        try:
    ...
    logger.info("Testing option instruments for NIFTY, %s, strikes %s", expiry_dates[0], strikes)
    ...
    if callable(get_opts):
        try:
            ...
            logger.error("Error retrieving instruments: %s", e)
            try:
                if handle_provider_error is not None:
                    handle_provider_error(
                        e,
                        component="tools.test_kite_connection",
                        index_name="NIFTY",
                        context={"op": "get_option_instruments"},
                    )
            except Exception:
                pass
            return 1
        if instruments:
            if isinstance(instruments, list):
                logger.info("✅ Got %s option instruments", len(instruments))
                sample = (
                    instruments[0].get('tradingsymbol')
