...
try:
    from src.error_handling import handle_collector_error as _handle_collector_error_import
except ImportError:
    _handle_collector_error_import = None  # type: ignore
...
def main():
    ...
    for index in indices:
        ...
        try:
            ...
            step = 100 if index in ("BANKNIFTY", "SENSEX") else 50
            ...
            if options_data:
                ...
                # Update options data with quote information (defensive on type)
                if not isinstance(quotes, dict):
                    quotes = {}
                for exchange, symbol in quote_instruments:
                    key = f"{exchange}:{symbol}"
                    q_any = quotes.get(key)
            ...
            try:
                if _handle_collector_error_import:
                    _handle_collector_error_import(e, component="tests.test_all_indices", index_name=index)
            except Exception:
                pass
