...
def test_no_datetime_utcnow():
    ...
    for path in repo_root.rglob('*.py'):
        ...
        if '.venv' in pstr or 'site-packages' in pstr:
            continue
        # Skip test files and external/upstream or backup mirrors
        if '/tests/' in pstr or '\\tests\\' in pstr:
            continue
        if '/external/' in pstr or '\\external\\' in pstr:
            continue
        parts = [p.lower() for p in path.parts]
        if any(p.startswith('_backup') for p in parts):
            continue
