...
try:
    from src.tools import token_manager as _token_manager_import
except ImportError:
    _token_manager_import = None  # type: ignore
try:
    from src.metrics.testing import force_new_metrics_registry as _force_new_metrics_registry_import
except ImportError:
    _force_new_metrics_registry_import = None  # type: ignore
try:
    from src.metrics import get_metrics as _get_metrics_import
except ImportError:
    _get_metrics_import = None  # type: ignore
try:
    from src.utils.output import get_output as _get_output_import
except ImportError:
    _get_output_import = None  # type: ignore
try:
    from src.metrics import setup_metrics_server as _setup_metrics_server_import
except ImportError:
    _setup_metrics_server_import = None  # type: ignore

# ---------------------------------------------------------------------------
# Global sane defaults for test environment (session)
#
# Purpose: Provide stable defaults that avoid surprise strict-mode failures
# when the developer has pre-seeded environment variables locally (e.g. from
# a .env file) or from the host OS. Tests that validate strict behavior set
# their own env overrides and remain unaffected.
# ---------------------------------------------------------------------------
@pytest.fixture(scope="session", autouse=True)
def _test_env_defaults():  # type: ignore
    # Keep this fixture minimal to avoid interfering with tests that assert
    # specific env deprecation behavior. Do not mutate G6_ENV_DEPRECATION_* here.
    # Prefer mock provider by default to keep tests hermetic
    os.environ.setdefault('G6_USE_MOCK_PROVIDER', '1')
    # Keep event logging enabled but default the path under local logs/ to avoid
    # writing into user profile paths on Windows when host env overrides exist.
    os.environ.setdefault('G6_EVENTS_LOG_PATH', str((__import__('pathlib').Path('logs') / 'events.test.log')))
    # No return value (fixture is autouse)
    yield

# Ensure EnvConfig reads fresh environment values per-test (it caches by design).
@pytest.fixture(autouse=True)
def _clear_env_cache_per_test():  # type: ignore
    try:
...
def _early_session_provision():  # type: ignore
    ...
    try:
        if _token_manager_import:
            _token_manager_import.load_env_vars()
    except Exception:
        pass
...
def _auto_metrics_reset(request):
    ...
    if is_truthy_env('G6_DISABLE_AUTOUSE_METRICS_RESET'):
        ...
        return
    try:
        if _force_new_metrics_registry_import:
            _force_new_metrics_registry_import(enable_resource_sampler=False)
    except Exception:
        try:
            if _get_metrics_import:
                _ = _get_metrics_import()
        except Exception:
            pass
...
def _auto_output_reset(request):
    ...
    # Reset before test
    try:
        if _get_output_import:
            _get_output_import(reset=True)
    except Exception:
        pass
    yield
...
def run_orchestrator_cycle(monkeypatch, tmp_path) -> Callable[[int, int, bool], dict]:
    ...
    def _run(cycles: int = 1, interval: int = 2, strict: bool = False) -> dict:
        ...
        try:
            ...
            try:
                if _setup_metrics_server_import:
                    _setup_metrics_server_import(reset=True)
            except Exception:
                pass
            try:
                ctx, _stop_metrics = bootstrap_runtime(config_path)  # type: ignore[misc]
