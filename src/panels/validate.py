"""Panels validation against JSON schemas.

Provides:
- Schema loading from data/schemas/panels/
- Bulk validation via verify_all_panels(panels_dir)
- Runtime validation with pluggable severity (off/warn/strict)
- Manifest hash verification (detect write-time corruption)
"""
...
def runtime_validate_panel(payload: dict[str, Any]) -> None:
    ...
    """
    mode = EnvConfig.get_str("G6_PANELS_VALIDATE", "warn").lower()
