"""status_reducer.py — Derive hierarchical statuses from per-expiry data.

Phase 10 enhancements: partial reason tokens & configurable coverage thresholds.
"""
...
def get_status_thresholds() -> tuple[float, float]:
    ...
    # Read directly from os.environ to reflect mid-test monkeypatches without relying on EnvConfig cache
    s_env = os.environ.get(ENV_STRIKE, '')
    f_env = os.environ.get(ENV_FIELD, '')
