...
# Error handling imports (moved from late imports)
from src.error_handling import handle_api_error, handle_data_error
...
class MetricsProcessor:
    ...
    def fetch_prometheus_metrics(self) -> dict[str, Any]:
        ...
        try:
            ...
            if response.status_code != 200:
                logger.warning("Prometheus server returned %s", response.status_code)
                return {}

            return self._parse_prometheus_text(response.text)

        except requests.exceptions.RequestException as e:
            logger.warning("Cannot connect to Prometheus metrics server: %s", e)
            try:
        ...
        for line in metrics_text.split('\n'):
            ...
            try:
                ...
                try:
                    handle_data_error(
                        e,
                        component="summary.metrics_processor",
                        context={"op": "parse_line", "line": line[:128]},
                    )
                except Exception:
                    pass
                continue
    ...
    def process_performance_metrics(self, prometheus_metrics: dict[str, Any]) -> PerformanceMetrics:
        ...
        return PerformanceMetrics(
            uptime_seconds=self._get_metric_value(
                prometheus_metrics, "g6_uptime_seconds"
            )
            or 0.0,
            collection_cycle_time=self._get_metric_value(
                prometheus_metrics, "g6_avg_cycle_time_seconds"
            )
            or 0.0,
            processing_time_per_option=self._get_metric_value(
                prometheus_metrics, "g6_processing_time_per_option_seconds"
            )
            or 0.0,
            api_response_time=self._get_metric_value(
                prometheus_metrics, "g6_api_latency_ema_ms"
            )
            or 0.0,
            options_processed_total=int(
                self._get_metric_value(
                    prometheus_metrics, "g6_options_processed_total"
                )
                or 0
            ),
            options_per_minute=self._get_metric_value(
                prometheus_metrics, "g6_options_per_minute"
            )
            or 0.0,
            cycles_completed=int(
                self._get_metric_value(
                    prometheus_metrics, "g6_collection_cycles_total"
                )
                or 0
            ),
            cycles_per_hour=self._get_metric_value(
                prometheus_metrics, "g6_cycles_per_hour"
            )
            or 0.0,
            api_success_rate=self._get_metric_value(
                prometheus_metrics, "g6_api_success_rate_percent"
            )
            or 0.0,
            collection_success_rate=self._get_metric_value(
                prometheus_metrics, "g6_collection_success_rate_percent"
            )
            or 0.0,
            data_quality_score=self._get_metric_value(
                prometheus_metrics, "g6_data_quality_score_percent"
            )
            or 0.0,
            memory_usage_mb=self._get_metric_value(
                prometheus_metrics, "g6_memory_usage_mb"
            )
            or 0.0,
            cpu_usage_percent=self._get_metric_value(
                prometheus_metrics, "g6_cpu_usage_percent"
            )
            or 0.0,
            disk_io_operations=int(
                self._get_metric_value(
                    prometheus_metrics, "g6_disk_io_operations_total"
                )
                or 0
            ),
            network_bytes_transferred=int(
                self._get_metric_value(
                    prometheus_metrics, "g6_network_bytes_transferred_total"
                )
                or 0
            ),
        )
    ...
    def process_index_metrics(self, prometheus_metrics: dict[str, Any]) -> dict[str, IndexMetrics]:
        ...
        for index in indices:
            ...
            last_collection_ts = self._get_index_metric_value(
                prometheus_metrics, "g6_index_last_collection_unixtime", index
            )
            ...
            index_metrics[index] = IndexMetrics(
                options_processed=int(
                    self._get_index_metric_value(
                        prometheus_metrics, "g6_index_options_processed_total", index
                    )
                    or 0
                ),
                avg_processing_time=self._get_index_metric_value(
                    prometheus_metrics, "g6_index_avg_processing_time_seconds", index
                )
                or 0.0,
                success_rate=self._get_index_metric_value(
                    prometheus_metrics, "g6_index_cycle_success_percent", index
                )
                or 0.0,
                last_collection_time=last_collection_time,
                atm_strike_current=self._get_index_metric_value(
                    prometheus_metrics, "g6_index_current_atm", index
                ),
                volatility_current=self._get_index_metric_value(
                    prometheus_metrics, "g6_index_implied_volatility", index
                ),
                current_cycle_legs=int(
                    self._get_index_metric_value(
                        prometheus_metrics, "g6_index_options_processed", index
                    )
                    or 0
                ),
                cumulative_legs=int(
                ...
                data_quality_score=self._get_index_metric_value(
                    prometheus_metrics, "g6_index_data_quality_score_percent", index
                )
                or 0.0,
                data_quality_issues=int(
                    self._get_index_metric_value(
                        prometheus_metrics, "g6_index_dq_issues_total", index
                    )
                    or 0
                ),
            )
    ...
    def process_storage_metrics(self, prometheus_metrics: dict[str, Any]) -> StorageMetrics:
        ...
        return StorageMetrics(
            csv_files_created=int(
                self._get_metric_value(prometheus_metrics, "g6_csv_files_created_total") or 0
            ),
            csv_records_written=int(
                self._get_metric_value(prometheus_metrics, "g6_csv_records_written_total") or 0
            ),
            csv_write_errors=int(
                self._get_metric_value(prometheus_metrics, "g6_csv_write_errors_total") or 0
            ),
            csv_disk_usage_mb=self._get_metric_value(
                prometheus_metrics, "g6_csv_disk_usage_mb"
            )
            or 0.0,
            csv_last_write_time=csv_last_write_time,
            influxdb_points_written=int(
                self._get_metric_value(
                    prometheus_metrics, "g6_influxdb_points_written_total"
                )
                or 0
            ),
            influxdb_write_success_rate=self._get_metric_value(
                prometheus_metrics, "g6_influxdb_write_success_rate_percent"
            )
            or 0.0,
            influxdb_connection_status=influxdb_status,
            influxdb_query_performance=self._get_metric_value(
                prometheus_metrics, "g6_influxdb_query_time_ms"
            )
            or 0.0,
            influxdb_last_write_time=influxdb_last_write_time,
            backup_files_created=int(
                self._get_metric_value(prometheus_metrics, "g6_backup_files_created_total") or 0
            ),
            last_backup_time=None,  # Would need separate timestamp metric
            backup_size_mb=self._get_metric_value(
                prometheus_metrics, "g6_backup_size_mb"
            )
            or 0.0,
        )
    ...
    def get_all_metrics(self, force_refresh: bool = False) -> PlatformMetrics:
        ...
        suppress_off_hours = EnvConfig.get_bool("G6_SUPPRESS_CLOSED_METRICS", True)
