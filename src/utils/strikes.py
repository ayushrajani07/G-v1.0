...
def build_strikes(
    ...
    *,
    ...
    # Build ITM and OTM lists using comprehensions for speed and clarity
    itm: list[float] = [float(atm - i * step) for i in range(1, _n_itm + 1)]
    otm: list[float] = [float(atm + i * step) for i in range(1, _n_otm + 1)]
    strikes: list[float] = itm + [float(atm)] + otm
