...
def _log_path() -> str:
    override = EnvConfig.get_str("G6_EVENTS_LOG_PATH", "")
    if override:
        return override
...
def events_enabled() -> bool:
    return not EnvConfig.get_bool("G6_EVENTS_DISABLE", False)
...
def configure_from_env() -> None:
    ...
    lvl = EnvConfig.get_str("G6_EVENTS_MIN_LEVEL", "debug").lower()
    _min_level_rank = _LEVEL_RANK.get(lvl, 0)
    try:
        raw_map = EnvConfig.get_str("G6_EVENTS_SAMPLE_MAP", "")
        if raw_map:
            _sampling_map = json.loads(raw_map)
    except Exception:  # pragma: no cover
        logger.warning("[events] Failed to parse G6_EVENTS_SAMPLE_MAP")
        _sampling_map = {}
    _sampling_default = EnvConfig.get_float("G6_EVENTS_SAMPLE_DEFAULT", 1.0)
    _recent_buffer_max = EnvConfig.get_int("G6_EVENTS_RECENT_MAX", _recent_buffer_max)
