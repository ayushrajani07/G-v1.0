...
try:
    ...
    seconds_until_market_open = None  # type: ignore
...
def kite_auth_validation(ctx) -> None:
    ...
    """
    if tm is None:
        logger.debug("[startup] token_manager module unavailable; skipping Kite auth validation")
        return
    ...
    for index, cfg in index_params.items():
        ...
        if trace:
            try:
                ...
                if raw_list:
                    logger.info(
                        "[startup] raw_expiries index=%s count=%d sample=%s",
                        index,
                        len(raw_list),
                        raw_list[:8],
                    )
