...
class MetricsRegistry:
    ...
    def _maybe_register(self, group: str, attr: str, metric_cls, name: str, documentation: str, labels: list[str] | None = None, **ctor_kwargs):  # type: ignore[override]
        ...
        try:
            ...
            except (AttributeError, TypeError, ValueError) as log_err:
                logger.debug(f"Failed to log _maybe_register error: {log_err}")
            return None
    ...
    def __init__(self):
        ...
        if _prof_enabled:
            ...
            def _prof_mark(label: str, started_at: float):  # type: ignore
                try:
                    ...
                    logger.debug(f"Failed to mark profile phase {label}: {e}")
        else:
        ...
        def _pt(label: str, **kw):
            ...
            except (AttributeError, TypeError, ValueError, IOError, OSError) as e:
                logger.debug(f"Failed to print trace for {label}: {e}")
        _pt('begin')
        ...
        def _step(label: str):  # local helper to record step start
            ...
            def _end(ok: bool = True, **info):
                try:
                    ...
                    logger.debug(f"Failed to record init trace for {label}: {e}")
            return _end
            ...
            if not hasattr(self, 'group_allowed'):
                def group_allowed(name: str) -> bool:  # type: ignore
                    try:
                        ...
                        logger.debug(f"group_allowed check failed for {name}, defaulting to True: {e}")
                        return True
        ...
        except (ImportError, AttributeError, TypeError, ValueError, KeyError) as _e:
            # Fallback uses canonical exported copy to avoid duplicated literal.
            logger.debug(f"Group gating configuration failed: {_e}")
            try:
                from .gating import CONTROLLED_GROUPS_FALLBACK as _CGF  # type: ignore
                CONTROLLED_GROUPS = set(_CGF)
            except (ImportError, AttributeError, TypeError) as fallback_err:
                logger.debug(f"Fallback groups import failed: {fallback_err}")
        ...
        try:
            ...
            logger.debug(f"Failed to set auto test-mode skip for provider seed: {e}")
        ...
        try:
            ...
            for _spec in METRIC_SPECS:
                try:
                    ...
                    logger.debug(f"Failed to register metric spec: {e}")
            for _spec in GROUPED_METRIC_SPECS:
                try:
                    _spec.register(self); scount += 1
                except (AttributeError, TypeError, ValueError, KeyError) as e:
                    logger.debug(f"Failed to register grouped metric spec: {e}")
            _end(ok=True, count=scount)
            _pt('spec_registration_ok', count=scount)
            if _prof_enabled:
                _prof_mark('spec_registration', _prof_t)
        except (ImportError, AttributeError, TypeError, ValueError, KeyError) as _e:  # pragma: no cover
            _end(ok=False, error=str(_e))
            _pt('spec_registration_fail', error=str(_e))
            try:
                logger.debug("Spec metric registration failed", exc_info=True)
            except (AttributeError, TypeError, ValueError) as log_err:
                logger.debug(f"Failed to log spec registration error: {log_err}")
            if _prof_enabled:
                _prof_mark('spec_registration', _prof_t)
        ...
        if _trace_simple:
            try:
                ...
                logger.debug(f"Failed to emit post-spec trace: {e}")
        ...
        if not hasattr(self, '_group_filters_log_emitted'):
            try:
                ...
                logger.debug(f"Failed to set _group_filters_log_emitted: {e}")
            ...
            try:
                ...
                if not hasattr(self, 'provider_mode'):
                    try:
                        ...
                        logger.debug(f"Failed to create provider_mode gauge: {e}")
                    ...
                    try:
                        ...
                        for child in list(child_map.values()):
                            try:
                                ...
                                logger.debug(f"Failed to zero provider_mode child: {e}")
                    except (AttributeError, TypeError, KeyError) as e:
                        logger.debug(f"Failed to access provider_mode child_map: {e}")
                    # Create primary label sample
                    try:
                        g.labels(mode='primary').set(1)  # type: ignore[attr-defined]
                    except (AttributeError, TypeError, ValueError, KeyError) as e:
                        logger.debug(f"Failed to set primary provider_mode label: {e}")
                ...
                if _t.time() > seed_deadline and _trace_simple:
                    try:
                        ...
                        logger.debug(f"Failed to print slow seed trace: {e}")
            except (AttributeError, TypeError, ValueError, ImportError) as e:
                logger.debug(f"Provider mode seeding failed: {e}")
        if _trace_simple:
        ...
        try:
            ...
            logger.debug(f"Failed to canonicalize counter aliases: {e}")
        if _trace_simple:
        ...
        try:
            if '_G6_METRICS_SUMMARY_EMITTED' not in globals():
                ...
                try:
                    ...
                    logger.debug(f"Failed to register summary note (first): {e}")
                # JSON variant
                try:
                    if _is_truthy_env('G6_METRICS_SUMMARY_JSON'):
                        if _emit_summary_json_import:
                            _emit_summary_json_import(
                            'metrics.registry',
                            [
                                ('families', fam_count),
                    ...
                    logger.debug(f"Failed to emit JSON summary: {e}")
                if _is_truthy_env('G6_METRICS_SUMMARY_HUMAN'):
                    try:
                        if _emit_human_summary_import:
                            _emit_human_summary_import(
                        ...
                        logger.debug(f"Failed to emit human summary: {e}")
                    else:
                        ...
                        try:
                            ...
                            logger.debug(f"Failed to register summary note (second): {e}")
        except (AttributeError, TypeError, ValueError, KeyError, ImportError) as e:
            logger.debug(f"Failed to emit metrics summary: {e}")
        ...
        try:  # pragma: no cover - logic verified via downstream tests
            ...
            def _force_panel_metric(attr: str, prom_name: str, ctor, labels: list[str], doc: str):  # noqa: ANN001
                ...
                if existing is not None:
                    try:
                        ...
                        logger.debug(f"Failed to check existing panel metric labels: {e}")
                        recreate = True
                if recreate:
                    try:
                        _R.unregister(existing)  # type: ignore[arg-type]
                    except (AttributeError, TypeError, ValueError, KeyError) as e:
                        logger.debug(f"Failed to unregister panel metric: {e}")
                if recreate or existing is None:
                    try:
                        ...
                        try: self._metric_groups[attr] = 'panel_diff'  # type: ignore[attr-defined]
                            ...
                            logger.debug(f"Failed to set metric group for {attr}: {e}")
                    except ValueError:
                        ...
                        if metric_obj is not None:
                            try: setattr(self, attr, metric_obj)
                                ...
                                logger.debug(f"Failed to bind existing panel metric {attr}: {e}")
                else:
                    if not hasattr(self, attr):
                        try: setattr(self, attr, existing)
                        except (AttributeError, TypeError) as e:
                            logger.debug(f"Failed to set existing panel metric {attr}: {e}")
            ...
            logger.debug(f"Failed to initialize panel diff metrics: {e}")
        ...
        for attr, ctor, name, doc, labels in _core_fallback:
            if not hasattr(self, attr):
                try:
                    ...
                    logger.debug(f"Failed to create core fallback metric {attr}: {e}")
        ...
        try:
            ...
            logger.debug(f"Index aggregate metrics initialization failed: {_e}")
            _end(ok=False, error=str(_e))
        ...
        if not hasattr(self, 'metric_group_state'):
            try:
                ...
                logger.debug(f"Failed to create metric_group_state gauge: {e}")
        ...
        try:
            ...
            try:
                ...
                logger.debug(f"Failed to initialize SLA placeholders: {e}")
            ...
            try:
                ...
                logger.debug(f"Failed to initialize fault budget: {e}")
            try:
                ...
                logger.debug(f"Failed to initialize provider failover placeholders: {e}")
            try:
                from .scheduler import init_scheduler_placeholders as _init_sched  # type: ignore
                _init_sched(self, self._group_allowed)
            except (ImportError, AttributeError, TypeError, ValueError) as e:
                logger.debug(f"Failed to initialize scheduler placeholders: {e}")
            try:
                from .adaptive import init_adaptive_placeholders as _init_adap  # type: ignore
                _init_adap(self, self._group_allowed)
            except (ImportError, AttributeError, TypeError, ValueError) as e:
                logger.debug(f"Failed to initialize adaptive placeholders: {e}")
        except (ImportError, AttributeError, TypeError, ValueError) as e:  # pragma: no cover
            try:
                logger.debug(f"init_always_on_placeholders failed: {e}", exc_info=True)
            except (AttributeError, TypeError, ValueError) as log_err:
                logger.debug(f"Failed to log placeholder init error: {log_err}")
        ...
        try:
            ...
            logger.debug(f"Performance metrics initialization failed: {_e}")
            _end(ok=False, error=str(_e))
        ...
        try:
            ...
            logger.debug(f"API call metrics initialization failed: {_e}")
            _end(ok=False, error=str(_e))
        ...
        try:
            ...
            logger.debug(f"Failed to create API fallback metrics: {e}")
        try:
            from .resource_category import init_resource_metrics as _res  # type: ignore
            _res(self)
        except (ImportError, AttributeError, TypeError, ValueError) as e:
            logger.debug(f"Failed to initialize resource metrics: {e}")
        try:
            from .cache_error import init_cache_error_metrics as _cache  # type: ignore
            _cache(self)
        except (ImportError, AttributeError, TypeError, ValueError) as e:
            logger.debug(f"Failed to initialize cache error metrics: {e}")
        try:
        ...
        logger.info("Initialized %s metrics for g6_platform", len(self.__dict__))
        ...
        try:
            if any(_env_str(k, '') for k in ("G6_CARDINALITY_SNAPSHOT", "G6_CARDINALITY_BASELINE")):
                # Ensure EnvConfig reflects latest env mutations across test phases
                try:
