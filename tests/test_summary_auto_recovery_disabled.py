...
try:
    from src.metrics import registry as _registry_import  # type: ignore
except ImportError:
    _registry_import = None  # type: ignore
...
def test_auto_recovery_disabled_no_force_full(monkeypatch, tmp_path):
    ...
    if _registry_import:
        try:
            m = getattr(_registry_import, 'events_full_recovery_total', None)
            if m is not None:
