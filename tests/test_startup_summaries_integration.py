...
try:
    from src.metrics import _reset_metrics_summary_state as _reset_metrics_summary_state_import  # type: ignore
except ImportError:
    _reset_metrics_summary_state_import = None  # type: ignore
try:
    from src.broker.kite.startup_summary import _reset_provider_summary_state as _reset_provider_summary_state_import  # type: ignore
except ImportError:
    _reset_provider_summary_state_import = None  # type: ignore
try:
    from src.orchestrator import bootstrap as _bootstrap_import  # type: ignore
except ImportError:
...
def test_all_summaries_once(monkeypatch, tmp_path):
    ...
    try:
        ...
        if _reset_metrics_summary_state_import:
            try:
                _reset_metrics_summary_state_import()
            except Exception:
                pass
        ...
        try:
            ...
            if _reset_provider_summary_state_import:
                try:
                    _reset_provider_summary_state_import()
        ...
        if _bootstrap_import:
            _bootstrap_import.bootstrap_runtime(str(cfg_path))

        # Force metrics registry init to emit metrics summary
        if _MetricsRegistry_import:
            try:
                _MetricsRegistry_import()
            except Exception:
                pass
