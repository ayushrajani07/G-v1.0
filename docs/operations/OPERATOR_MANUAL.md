...
- Optional: metrics endpoint reachable (default `http://localhost:9108/metrics` when metrics enabled)
...
### 3.1 Metrics Endpoint (/metrics)
The platform exposes an embedded `/metrics` endpoint in Prometheus text exposition format.

Default URL: `http://localhost:9108/metrics` (override via `G6_METRICS_PORT` / `G6_METRICS_HOST`). Confirm with browser or curl.
...
Shows per-cycle snapshot: success %, duration, throughput, memory. If an index shows NA for a field, treat it as a wiring/coverage gap and confirm health via the JSON runtime status + `/metrics` endpoint.
...
| All indices show NA in live panel | Wiring gap / field not emitted | Use JSON runtime status + `/metrics` endpoint for detail; treat as non-fatal if collection succeeds |
...
- Limit network ingress to the metrics endpoint where possible.
...
Use `dump_metrics()` (test / debugging) or scrape the `/metrics` endpoint to confirm presence.
...
curl http://localhost:9108/metrics | Select-String g6_collection_cycles_total
```
