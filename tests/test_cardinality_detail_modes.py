...
def test_detail_mode_aggregate_blocks_all(monkeypatch, metrics_port):
...
def test_detail_mode_band_window(monkeypatch, metrics_port):
    monkeypatch.setenv('G6_METRICS_CARD_ENABLED','1')
...
def test_detail_mode_full_no_extra_gating(monkeypatch, metrics_port):
    monkeypatch.setenv('G6_METRICS_CARD_ENABLED','1')
    monkeypatch.setenv('G6_DETAIL_MODE_BAND_ATM_WINDOW','2')
    metrics,_ = setup_metrics_server(port=metrics_port, reset=True)
    mgr = get_cardinality_manager()
