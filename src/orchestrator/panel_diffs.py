...
_EGRESS_FROZEN = EnvConfig.get_bool('G6_EGRESS_FROZEN', False)
...
def emit_panel_artifacts(status: dict[str, Any], *, status_path: str) -> None:
    ...
    if _EGRESS_FROZEN:
        return
    if not EnvConfig.get_bool('G6_PANEL_DIFFS', False):
        return
    import time as _t
    start_time = _t.time()
    global _state
    # Config
    full_interval = EnvConfig.get_int('G6_PANEL_DIFF_FULL_INTERVAL', 30)
    nest_depth = EnvConfig.get_int('G6_PANEL_DIFF_NEST_DEPTH', 1)
    if nest_depth < 0:
        nest_depth = 0
    max_keys = EnvConfig.get_int('G6_PANEL_DIFF_MAX_KEYS', 0)
    if max_keys < 0:
        max_keys = 0
            ...
            if k not in a:
                ...
                if a[k] != v:
                    if depth > 0 and isinstance(a[k], dict) and isinstance(v, dict):
                        ...
                        if (
                            nested_diff.get("added")
                            or nested_diff.get("removed")
                            or nested_diff.get("changed")
                            or nested_diff.get("nested")
                        ):
