...
# Optional timeutils imports
try:
    from src.utils.timeutils import utc_now, ensure_utc_helpers
except ImportError:
    utc_now = None  # type: ignore
    ensure_utc_helpers = None  # type: ignore
...
class InfluxSink:
    ...
    def __init__(
        ...
        self._buffer: InfluxBufferManager | None = None
        self._breaker = InfluxCircuitBreaker(
        ...
        try:
            ...
            eff_flush = _env_float(
                'G6_INFLUX_FLUSH_INTERVAL',
                float(flush_interval) if flush_interval is not None else 1.0,
            )
            ...
            logger.info("InfluxDB sink initialized with bucket: %s", bucket)
            ...
            logger.error("Error initializing InfluxDB client: %s", e)
            get_error_handler().handle_error(
                e,
        ...
        try:
            if _env_bool('G6_ADAPTIVE_CB_INFLUX', False):
                if hasattr(self, 'write_options_data'):
                    self.write_options_data = cast(
                        Any,
                        circuit_protected('influx.write_options_data')(self.write_options_data),
                    )
                if hasattr(self, 'write_overview_snapshot'):
                    self.write_overview_snapshot = cast(
                        Any,
                        circuit_protected('influx.write_overview_snapshot')(self.write_overview_snapshot),
                    )
                if hasattr(self, 'write_cycle_stats'):
                    self.write_cycle_stats = cast(
                        Any,
                        circuit_protected('influx.write_cycle_stats')(self.write_cycle_stats),
                    )
        except Exception as e:
            get_error_handler().handle_error(
                e,
    ...
    def write_options_data(
        ...
        try:
            ...
            if not options_data:
                logger.warning("No options data to write for %s %s", index_symbol, expiry_date)
                return
            ...
            logger.error("Error writing options data to InfluxDB: %s", e)
            get_error_handler().handle_error(
                e,
                category=ErrorCategory.DATABASE,
                severity=ErrorSeverity.HIGH,
    ...
    def write_overview_snapshot(
        ...
        Tags: index
        Fields:
            - pcr_this_week, pcr_next_week
            - pcr_this_month, pcr_next_month
            - day_width
        """
        if not self.client or not self.write_api:
            return
        try:
            ...
            logger.error("Error writing overview snapshot to InfluxDB: %s", e)
            get_error_handler().handle_error(
                e,
                category=ErrorCategory.DATABASE,
                severity=ErrorSeverity.HIGH,
    ...
    def write_cycle_stats(
        self,
        cycle: int,
        elapsed: float,
        success_rate: float | None,
        options_last: int | None,
        per_index: dict[str, int] | None,
        timestamp: datetime | None = None,
    ) -> None:
        ...
        Fields:
            ...
            - options_last_cycle (int)
            - per-index option counts as fields: options_<INDEX>
        """
        if not self.client or not self.write_api:
            return
        try:
            ...
            if timestamp is None:
                if ensure_utc_helpers is not None:
                    try:
                        _utc_now, _iso = ensure_utc_helpers()
                    except Exception:
            ...
            if per_index:
                def _safe_add_field(p: Any, key: str, value: int) -> Any:
                    try:
                        return p.field(f"options_{key}", int(value))
                    except Exception:
                        return p
                for k, v in per_index.items():
                    point = _safe_add_field(point, k, v)
            point = point.time(timestamp)
            if getattr(self, "_breaker", None) is None or self._breaker.allow():
            ...
            logger.debug("Failed to write cycle stats: %s", e)
            get_error_handler().handle_error(
                e,
                category=ErrorCategory.DATABASE,
                severity=ErrorSeverity.LOW,
