...
try:
    ...
    get_recent_events = None  # type: ignore
...
def _gather_recent_events() -> tuple[list[dict], int | None]:
    limit = EnvConfig.get_int('G6_CATALOG_EVENTS_LIMIT', 20)
    if limit <= 0:
        return [], None
    include_ctx = EnvConfig.get_bool('G6_CATALOG_EVENTS_CONTEXT', True)
    events: list[dict] = []
    last_seq: int | None = None
    # Prefer in-process event API if available (gives us recent buffer without reading disk again)
    try:
        if get_recent_events:
...
def build_catalog(*, runtime_status_path: str, csv_dir: str = "data/g6_data") -> dict[str, Any]:
    ...
    try:
        events_path = EnvConfig.get_str('G6_EVENTS_LOG_PATH', os.path.join('logs','events.log'))
        cycles: list[int] = []
        try:
