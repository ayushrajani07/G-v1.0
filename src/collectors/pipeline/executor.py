...
from src.metrics import get_metrics, MetricsRegistry
                    ...
                    if _rw_size_env > 0:
                        try:
                            ...
                            if not hasattr(execute_phases, '_rolling_window'):
                                ...
                                execute_phases._rolling_window = _deque(maxlen=_rw_size_env)  # type: ignore[attr-defined]
                            window = cast(_deque[int], execute_phases._rolling_window)  # type: ignore[attr-defined]
        ...
        if _ppds is not None:  # histogram
            try:
                ...
                if not hasattr(hist, '_g6_buckets_overridden'):
                    # Use env_adapter which already wraps EnvConfig
