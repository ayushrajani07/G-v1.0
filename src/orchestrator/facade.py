...
def _select_mode(requested: str) -> str:
    if requested not in ("auto","legacy","pipeline"):
        logger.warning("facade_invalid_mode_defaulting mode=%s", requested)
        requested = "auto"
    if requested == "auto":
        ...
        if EnvConfig.get_bool("G6_LEGACY_COLLECTOR", False):
            return "legacy"
        return "pipeline"
    if requested == "legacy":
        return "legacy"
...
def run_collect_cycle(index_params, providers, csv_sink, influx_sink, metrics=None, *,
    ...
    try:
        ...
        if pipe_idx != leg_idx:
            ...
            if EnvConfig.get_bool("G6_FACADE_PARITY_STRICT", False):
