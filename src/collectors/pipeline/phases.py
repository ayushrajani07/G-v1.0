...
# Modular helper imports (primary paths)
from src.collectors.modules.expiry_helpers import resolve_expiry, fetch_option_instruments, enrich_quotes
...
# Legacy fallback imports (for backward compatibility)
from src.collectors.unified_collectors import _resolve_expiry, _fetch_option_instruments, _enrich_quotes
...
def phase_resolve(ctx: Any, state: ExpiryState) -> ExpiryState:
    ...
    try:
        try:
            try:
                expiry_date = resolve_expiry(state.index, state.rule, ctx.providers, None, True)
...
def phase_fetch(ctx: Any, state: ExpiryState, precomputed_strikes: Sequence[float] | Iterable[float]) -> ExpiryState:
    ...
    try:
        ...
        try:
            ...
            try:
                raw_instruments = fetch_option_instruments(
        ...
        if not state.instruments and not used_unified:
            ...
            try:  # pragma: no cover - defensive
                ...
                raw2 = _try_call(_fetch_option_instruments, variants2)
...
def phase_iv(ctx: Any, state: ExpiryState) -> ExpiryState:
    ...
    if run_iv_estimation is None:
        meta.update({'import_error': 'module_unavailable'})
    else:
        try:
            meta['attempted'] = True
            try:
