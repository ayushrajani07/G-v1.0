...
def test_memory_gauge_emitted(monkeypatch):
    ...
    fake_prom = types.SimpleNamespace(
        ...
        Counter=lambda *a, **k: DummyGauge(),
        Histogram=lambda *a, **k: DummyGauge(),
        Summary=lambda *a, **k: DummyGauge()
    )
    ...
    result = pl.run_pipeline({}, providers=types.SimpleNamespace(get_atm_strike=lambda x: None), csv_sink=None, metrics=metrics)
