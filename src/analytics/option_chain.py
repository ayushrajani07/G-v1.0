...
class OptionChainAnalytics:
    ...
    def fetch_option_chain(
        ...
        try:
            ...
            if hasattr(self.provider, 'get_option_instruments'):
                try:
                    ...
                    logger.error("Provider get_option_instruments failed: %s", e)
            else:
            ...
            logger.error("Error fetching option instruments: %s", e)
        ...
        if option_keys:
            try:
                ...
                logger.error("Error fetching quotes for option chain: %s", e)
    ...
    def calculate_pcr(
        ...
        if option_chain.empty:
            logger.debug("Empty option chain for %s %s; PCR defaults to 0", index_symbol, expiry_date)
    ...
    def calculate_max_pain(
        ...
        if option_chain.empty:
            logger.debug("Empty option chain for max pain calculation %s %s; returning ATM %s", index_symbol, expiry_date, atm_strike)
            return atm_strike
