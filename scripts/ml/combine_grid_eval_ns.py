...
OUT_DIR = ROOT / "results" / "grid" / "COMBINED_NS" / "this_week" / "0"
...
def main():
    ...
    for r in all_rows:
        # Include optional scale if present; default to 1.0
        try:
            scale = float(str(r.get("scale", "1.0")))
        except Exception:
            scale = 1.0
        # Group by ANN knobs if present to avoid mixing ANN and exact
        try:
        ...
        try:
            ...
            ann_max = None
        key = (int(str(r["horizon"])), int(str(r["window"])) , int(str(r["k"])), str(r["mode"]), scale, ann_use, ann_space, ann_max)
        groups[key].append(r)

    out_rows = []
    for (h, win, k, mode, scale, ann_use, ann_space, ann_max), rows in sorted(groups.items()):
        n = 0
        cov_sum = bw_sum = mae_sum = bias_sum = samples_sum = 0.0
        ann_tw_sum = ann_sh_sum = lat_sum = 0.0
        base_lat_sum = speedup_sum = q50_mad_sum = build_ms_sum = mem_bytes_sum = prune_ratio_sum = effect_sum = 0.0
        cnt_cov = cnt_bw = cnt_mae = cnt_bias = cnt_samples = 0
        for r in rows:
