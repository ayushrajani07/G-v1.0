...
try:
    ...
    get_output = None  # type: ignore

try:
    from src.panels.helpers import compute_status_and_reason
except ImportError:
    compute_status_and_reason = None  # type: ignore
...
def _ensure_panels_sink_active() -> Any | None:
    ...
    if get_output is None:
        return None
    sinks_env = EnvConfig.get_str("G6_OUTPUT_SINKS", "stdout,logging")
    ...
    if "panels" not in tokens:
        ...
        if not EnvConfig.get_str("G6_PANELS_DIR", ""):
    ...
    *,
    ...
    if emit_stream:
        for idx in indices:
            ...
            if compute_status_and_reason is not None:
                try:
                    status, reason = compute_status_and_reason(
                        success_pct=sr_int,
                        legs=legs,
                        style='panels',
                    )
                except Exception:
                    if sr_int is None or (isinstance(sr_int, int) and sr_int >= 95):
                        status = "OK"
                    elif isinstance(sr_int, int) and sr_int >= 80:
                        status = "WARN"
                    else:
                        status = "ERROR"
                    reason = None
