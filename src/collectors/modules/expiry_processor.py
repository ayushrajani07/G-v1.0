...
# Optional imports for expiry_processor late import elimination (Batch 27)
try:
    from math import isnan
except ImportError:
    isnan = None  # type: ignore

# Define _emit_zero_data_struct as optional (may not exist in unified_collectors)
_emit_zero_data_struct: Callable[..., None] | None = None  # noqa: F811

try:
    from src.collectors.unified_collectors import _trace
except ImportError:
    _trace = None  # type: ignore
try:
    from src.collectors.modules.coverage_eval import coverage_metrics, field_coverage_metrics
except ImportError:
    coverage_metrics = None  # type: ignore
    field_coverage_metrics = None  # type: ignore
try:
    from src.collectors.helpers.status_reducer import compute_expiry_status
except ImportError:
    compute_expiry_status = None  # type: ignore
try:
    from src.collectors.modules.data_quality_bridge import run_option_quality, run_expiry_consistency
except ImportError:
    run_option_quality = None  # type: ignore
    run_expiry_consistency = None  # type: ignore
try:
    from src.collectors.modules.prefilter_flow import run_prefilter_clamp
except ImportError:
    run_prefilter_clamp = None  # type: ignore
try:
    from src.collectors.modules.error_bridge import report_no_instruments
except ImportError:
    report_no_instruments = None  # type: ignore
try:
    from src.collectors.modules.data_quality_flow import apply_data_quality
except ImportError:
    apply_data_quality = None  # type: ignore
try:
    from src.collectors.modules.preventive_validate import run_preventive_validation
except ImportError:
    run_preventive_validation = None  # type: ignore
try:
    from src.collectors.pipeline.shadow import run_shadow_pipeline
except ImportError:
    run_shadow_pipeline = None  # type: ignore
try:
    from src.collectors.pipeline.recovery import DefaultRecoveryStrategy
except ImportError:
    DefaultRecoveryStrategy = None  # type: ignore
try:
    from src.metrics.adapter import MetricsAdapter
except ImportError:
    MetricsAdapter = None  # type: ignore
try:
    from src.collectors.modules.iv_estimation import run_iv_estimation
except ImportError:
    run_iv_estimation = None  # type: ignore
try:
    from src.collectors.modules.greeks_compute import run_greeks_compute
except ImportError:
    run_greeks_compute = None  # type: ignore
try:
    from src.collectors.cycle_context import ExpiryContext
except ImportError:
    ExpiryContext = None  # type: ignore
try:
    from src.collectors.modules.persist_flow import run_persist_flow
except ImportError:
    run_persist_flow = None  # type: ignore
try:
    from src.collectors.helpers.persist import persist_with_context
except ImportError:
    persist_with_context = None  # type: ignore
try:
    from src.collectors.modules.status_finalize_core import finalize_expiry
except ImportError:
    finalize_expiry = None  # type: ignore
try:
    from src.collectors.modules.adaptive_adjust import adaptive_post_expiry
except ImportError:
    adaptive_post_expiry = None  # type: ignore
try:
...
try:
    ...
    build_expiry_snapshot = None  # type: ignore
try:
    from src.collectors.modules.formatters import format_concise_expiry_row
except ImportError:
    format_concise_expiry_row = None  # type: ignore
try:
    from src.collectors.settings import CollectorSettings as _CollectorSettingsImport
except ImportError:
    _CollectorSettingsImport = None  # type: ignore
...
else:  # runtime tolerant import - use module-level import
    if _CollectorSettingsImport:
        CollectorSettings = _CollectorSettingsImport  # type: ignore
...
def process_expiry(
    ...
    **legacy_kwargs: Any,
    ...
    global _trace
    if _trace is None:  # Module-level optional import (Phase 3)
    ...
    if coverage_metrics:  # Module-level optional import (Phase 3)
        _cov_metrics_raw = coverage_metrics
    if field_coverage_metrics:  # Module-level optional import (Phase 3)
        _field_cov_metrics_raw = field_coverage_metrics
    if not _cov_metrics_raw or not _field_cov_metrics_raw:
    ...
    def _classify_expiry_result(*_a: Any, **_k: Any) -> str:  # legacy stub
        return 'OK'
    if not compute_expiry_status:  # Module-level optional import (Phase 3)
        ...
        _compute_expiry_status = compute_expiry_status
    ...
    def _run_option_quality(dq: Any, options_data: dict[str, Any]) -> tuple[dict[str, Any], list[str]]:
        if not run_option_quality:  # Module-level optional import (Phase 3)
            return {}, []
        try:
            res = run_option_quality(dq, options_data)
    ...
    def _run_expiry_consistency(dq: Any, options_data: dict[str, Any], index_price: float | None, expiry_rule: str | None) -> list[Any]:
        if not run_expiry_consistency:  # Module-level optional import (Phase 3)
            return []
        try:
    ...
    try:
        ...
        try:
            if instruments:
                if run_prefilter_clamp:  # Module-level optional import (Phase 3)
        ...
        if dq_checker and dq_enabled and enriched_data:
            if apply_data_quality:  # Module-level optional import (Phase 3)
                try:
                    apply_data_quality(
                        dq_checker,
                        dq_enabled,
        ...
        with ctx.time_phase('preventive_validate'):
            ...
            if not prevent_report.get('ok', True):
                logger.warning("Preventive validation flagged issues for %s %s: issues=%s dropped=%s kept=%s", index_symbol, expiry_rule, prevent_report.get('issues'), prevent_report.get('dropped_count'), prevent_report.get('post_enriched_count'))
            else:
                logger.debug("Preventive validation ok %s %s kept=%s dropped=%s", index_symbol, expiry_rule, prevent_report.get('post_enriched_count'), prevent_report.get('dropped_count'))
        ...
        try:
            if collector_settings is not None and getattr(collector_settings, 'pipeline_v2_flag', False):
                ...
                if run_shadow_pipeline:  # Module-level optional import (Phase 3)
                    run_shadow_pipeline(
                        ctx,
                        collector_settings,
                        index=index_symbol,
        ...
        if (not enriched_data and _orig_enriched_snapshot):
            ...
            if salvage_only:
                if use_recovery_strategy:
                    if DefaultRecoveryStrategy:  # Module-level optional import (Phase 3)
                        try:
        ...
        if not ExpiryContext:  # Module-level optional import (Phase 3)
            raise ImportError("ExpiryContext not available")
        ...
        with ctx.time_phase('persist_and_metrics'):
            ...
            class _PersistFailSurrogate:  # local minimal substitute
                ...
                failed: bool = True
            persist_result: Any = _PersistFailSurrogate()
            
            if run_persist_flow:  # Module-level optional import (Phase 3)
                try:
        ...
        try:
            if precomputed_strikes:
                ...
                step_val = None
            if finalize_expiry:  # Module-level optional import (Phase 3)
                try:
        ...
        if _domain_models_enabled and enriched_data and build_snapshots:
            if OptionQuote:  # Module-level optional import (Phase 3)
                try:
                    mapped_cnt = 0
        ...
        if build_snapshots:
            if build_expiry_snapshot:  # Module-level optional import (Phase 3)
                try:
        ...
        logger.error("Error collecting data for %s %s: %s", index_symbol, expiry_rule, e)
