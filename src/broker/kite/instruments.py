...
try:
    from src.broker.kite_provider import _timed_call
except ImportError:
...
def fetch_instruments(provider: KiteProvider, exchange: str, force_refresh: bool = False) -> list[dict[str, Any]]:
    ...
    try:
        ...
        def _fetch():
            if provider._api_rl:
                provider._api_rl()
            # Avoid circular import fragility: provide a local fallback timed_call
            def _timed_call_fallback(fn, timeout: float):
            ...
            timed = _timed_call or _timed_call_fallback
            return timed(lambda: provider.kite.instruments(), timeout)  # type: ignore[arg-type]
        ...
        if isinstance(raw, list) and not raw:
            ...
            try:
                ...
                if provider._rl_fallback():
                    logger.debug("empty_retry_failed exch=%s err=%s", exch, _re)
        ...
        if _is_auth_error(e) or str(e) == 'kite_auth_failed':
            ...
            if provider._rl_fallback():
                logger.debug("Instrument fetch failed, using synthetic: %s", e)
        try:
