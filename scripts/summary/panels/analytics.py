...
def _create_analytics_panel(status: dict[str, Any] | None, compact: bool, low_contrast: bool) -> Any:
    ...
    def format_price(val: Any) -> str:
        ...
        try:
            ...
            logging.warning("Error formatting price %s: %s", val, e)
            return "—"
    ...
    def get_pcr_sentiment(pcr: Any) -> str:
        ...
        try:
            ...
            logging.warning("Error determining PCR sentiment for %s: %s", pcr, e)
            return "Unknown"
    ...
    try:
        ...
        logging.warning("Error reading panels JSON for analytics: %s", e)
    ...
    try:
        ...
        logging.warning("Error extracting analytics from status: %s", e)
        data = None
    ...
    try:
        ...
        logging.warning("Error extracting indices data: %s", e)
        indices = {}
        ...
        try:
                ...
                for idx_name, max_pain in max_pain_data.items():
                    ...
                    try:
                        ...
                        try:
                            ...
                            logging.warning("Error calculating distance for %s: %s", idx_name, e)
                            distance = "—"
                        ...
                        try:
                            ...
                            logging.warning("Error formatting PCR: %s", e)
                        ...
                        logging.warning("Error processing analytics row for %s: %s", idx_name, e)
            ...
            # If no max pain data but have global PCR
                ...
                try:
                    ...
                    for idx_name in major_indices:
                        ...
                        if idx_name in indices:
                            try:
                                ...
                                logging.warning("Error processing major index %s: %s", idx_name, e)
                                tbl.add_row("—", "—", "—", "—", "—")
                                shown += 1
                except Exception as e:
                    handle_ui_error(
                    ...
                    logging.warning("Error processing major indices section: %s", e)
            ...
            logging.error("Error processing analytics data: %s", e)
    ...
    try:
        if isinstance(data, dict) and isinstance(data.get("pcr"), (int, float)):
            ...
            if not (pcr != pcr):  # NaN check
                ...
                try:
                    ...
                    logging.warning("Error formatting PCR footer: %s", e)
        ...
        logging.warning("Error creating analytics footer: %s", e)
