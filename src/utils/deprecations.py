...
def _normalize_suppression_env() -> None:
    """Map legacy suppression env vars to unified G6_SUPPRESS_DEPRECATIONS.
     ...
     Precedence rules:
        1. If G6_SUPPRESS_DEPRECATIONS already set, legacy keys are ignored
            (one-time notice if verbose).
        2. Else if any legacy key truthy, set G6_SUPPRESS_DEPRECATIONS to that
            truthy value and emit a deprecation warning.
        3. Conflicting legacy keys (mixed truthy/falsey) resolve to truthy if
            any truthy present.
    """
    if "G6_SUPPRESS_DEPRECATIONS" in os.environ:
        # Optionally log if verbose deprecations enabled and legacy still present.
        if any(k in os.environ for k in _LEGACY_SUPPRESS_VARS) and EnvConfig.get_bool("G6_VERBOSE_DEPRECATIONS", False):
            try:
                logger.warning(
                    "deprecations.suppress_legacy_ignored: legacy suppression env vars present "
                    "but overridden by G6_SUPPRESS_DEPRECATIONS"
                )
            except Exception:
                pass
        return
    ...
    for key in _LEGACY_SUPPRESS_VARS:
        if key in os.environ:
            ...
            if EnvConfig.get_bool(key, False):
                legacy_truthy = True
    if not legacy_set:
        return
    if legacy_truthy:
    ...
    try:
        msg = (
            f"Legacy suppression env vars ({', '.join(legacy_set)}) are deprecated; "
            f"use G6_SUPPRESS_DEPRECATIONS=1 instead"
        )
        warnings.warn(msg, DeprecationWarning, stacklevel=2)
        logger.warning(
            "deprecations.suppress_legacy_mapped legacy=%s mapped_value=%s",
            ",".join(legacy_set),
            EnvConfig.get_str("G6_SUPPRESS_DEPRECATIONS", "0"),
        )
...
def emit_deprecation(
    ...
    *,
    ...
    """
    if EnvConfig.get_bool("G6_SUPPRESS_DEPRECATIONS", False) and not critical:
        return
    verbose = EnvConfig.get_bool("G6_VERBOSE_DEPRECATIONS", False)
...
def check_pipeline_flag_deprecation() -> None:
    if EnvConfig.get_str('G6_PIPELINE_COLLECTOR', '') != "":
        emit_deprecation(
            'G6_PIPELINE_COLLECTOR',
            (
                'G6_PIPELINE_COLLECTOR is deprecated: pipeline is default. '
                'Remove this flag; use G6_LEGACY_COLLECTOR=1 to force legacy.'
            ),
        )
